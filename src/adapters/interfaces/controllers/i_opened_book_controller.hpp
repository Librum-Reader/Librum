...
/**
 * The OpenedBookController class is set up to work on the currently opened
 * book. It has access to the IOpenedBookService interface, which makes it
 * possible to get information about the opened book such as getting its
 * highlights, the underyling FzDocument and similar.
 */
class ADAPTERS_EXPORT IOpenedBookController : public QObject
{
    Q_OBJECT
...
public:
    virtual ~IOpenedBookController() noexcept = default;
    ...
    virtual const domain::entities::Highlight* getHighlightAtPoint(
        const QPointF& point, int page, float zoom) const = 0;
...
signals:
    ...
    void goToBookmarkPosition(int pageNumber, int offsetY);
