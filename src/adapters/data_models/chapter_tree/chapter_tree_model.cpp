...
ChapterTreeModel::ChapterTreeModel(QObject* parent) :
    ...
    m_rootNode(new ChapterTreeNode("Title"))
{
    // Test data
...
QModelIndex ChapterTreeModel::index(int row, int column,
...
{
    if(parent.isValid())
        return static_cast<ChapterTreeNode*>(parent.internalPointer())
            ->columnCount();
...
QVariant ChapterTreeModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const
{
    Q_UNUSED(section);
