    ...
    m_isThumbnail(false)
{
    ...
    connect(this, &QQuickItem::windowChanged, m_redrawTimer,
            [this]()
            {
                m_redrawTimer->start();
            });
...
void PageItem::setFlickable(QQuickItem* flickable)
{
    ...
    if(flickable)
    {
        // QQuickFlickable is not exported so we need the old-style connects
        // here
        connect(flickable, SIGNAL(contentXChanged()), this,
                SLOT(contentXChanged()));  // clazy:exclude=old-style-connect
        connect(flickable, SIGNAL(contentYChanged()), this,
                SLOT(contentYChanged()));  // clazy:exclude=old-style-connect
    }
...
void PageItem::setDocument(DocumentItem* doc)
{
    ...
    m_documentItem = doc;
    Observer* observer = m_isThumbnail
                             ? m_documentItem.data()->thumbnailObserver()
                             : m_documentItem.data()->pageviewObserver();
    connect(observer, &Observer::pageChanged, this, &PageItem::pageHasChanged);
    connect(doc->document()->bookmarkManager(),
            &Okular::BookmarkManager::bookmarksChanged, this,
            &PageItem::checkBookmarksChanged);
    setPageNumber(0);
...
{
    if((m_page && m_viewPort.pageNumber == number) || !m_documentItem ||
       !m_documentItem.data()->isOpened() || number < 0)
    {
        return;
    }
...
void PageItem::setBookmarked(bool bookmarked)
{
    ...
    if(bookmarked)
    {
        m_documentItem.data()->document()->bookmarkManager()->addBookmark(
            m_viewPort);
    }
    else
    {
        m_documentItem.data()->document()->bookmarkManager()->removeBookmark(
            m_viewPort.pageNumber);
    }
...
QStringList PageItem::bookmarks() const
{
    ...
    const KBookmark::List pageMarks =
        m_documentItem.data()->document()->bookmarkManager()->bookmarks(
            m_viewPort.pageNumber);
...
void PageItem::goToBookmark(const QString& bookmark)
{
    Okular::DocumentViewport viewPort(
        QUrl::fromUserInput(bookmark).fragment(QUrl::FullyDecoded));
    setPageNumber(viewPort.pageNumber);

    // Are we in a flickable?
    if(m_flickable)
    {
        // normalizedX is a proportion, so contentX will be the difference
        // between document and viewport times normalizedX
        m_flickable.data()->setProperty(
            "contentX", qMax((qreal)0, width() - m_flickable.data()->width()) *
                            viewPort.rePos.normalizedX);

        m_flickable.data()->setProperty(
            "contentY",
            qMax((qreal)0, height() - m_flickable.data()->height()) *
...
QPointF PageItem::bookmarkPosition(const QString& bookmark) const
{
    ...
    return QPointF(qMax((qreal)0, width() - m_flickable.data()->width()) *
                       viewPort.rePos.normalizedX,
                   qMax((qreal)0, height() - m_flickable.data()->height()) *
                       viewPort.rePos.normalizedY);
}
...
void PageItem::removeBookmarkAtPos(qreal x, qreal y)
{
    ...
    if(m_bookmarked && m_documentItem.data()
                               ->document()
                               ->bookmarkManager()
                               ->bookmarks(m_viewPort.pageNumber)
                               .count() == 0)
    {
        m_bookmarked = false;
        Q_EMIT bookmarkedChanged();
...
{
    m_documentItem.data()->document()->bookmarkManager()->removeBookmark(
        Okular::DocumentViewport(bookmark));
    Q_EMIT bookmarksChanged();
}
...
QSGNode* PageItem::updatePaintNode(QSGNode* node,
...
void PageItem::requestPixmap()
{
    ...
    // Here we want to request the pixmap for the page, but it may happen that
    // the page already has the pixmap, thus requestPixmaps would not trigger
    // pageHasChanged and we would not call paint. Always call paint, if we
    // don't have a pixmap it's a noop. Requesting a page that already has a
    // pixmap is also almost a noop. Ideally we would do one or the other but
    // for now this is good enough
    paint();
    {
        auto request = new Okular::PixmapRequest(
            observer, m_viewPort.pageNumber, width() * dpr, height() * dpr,
            priority, Okular::PixmapRequest::Asynchronous);
        request->setNormalizedRect(Okular::NormalizedRect(0, 0, 1, 1));
        const Okular::Document::PixmapRequestFlag prf =
...
void PageItem::paint()
{
    ...
    PagePainter::paintPageOnPainter(&p, m_page, observer, flags, width(),
                                    height(), limits);
    p.end();
...
void PageItem::pageHasChanged(int page, int flags)
{
    if(m_viewPort.pageNumber == page)
    {
        ...
        {
            // if pixmaps have updated, just repaint .. don't bother updating
            // pixmaps AGAIN
            paint();
        }
        else
...
void PageItem::checkBookmarksChanged()
{
    ...
    bool newBookmarked =
        m_documentItem.data()->document()->bookmarkManager()->isBookmarked(
            m_viewPort.pageNumber);
...
void PageItem::contentXChanged()
{
    ...
    m_viewPort.rePos.normalizedX =
        m_flickable.data()->property("contentX").toReal() /
        (width() - m_flickable.data()->width());
}
...
void PageItem::contentYChanged()
{
    ...
    m_viewPort.rePos.normalizedY =
        m_flickable.data()->property("contentY").toReal() /
        (height() - m_flickable.data()->height());
}
