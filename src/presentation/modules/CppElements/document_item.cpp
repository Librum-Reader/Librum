...
{
    qmlRegisterUncreatableType<TOCModel>(
        "org.kde.okular.private", 1, 0, "TOCModel",
    ...
    connect(m_document, &Okular::Document::searchFinished, this,
            &DocumentItem::searchFinished);
    connect(m_document->bookmarkManager(),
    ...
    connect(m_document, &Okular::Document::warning, this,
            &DocumentItem::warning);
...
void DocumentItem::openUrl(const QUrl& url, const QString& password)
{
    ...
    QUrl realUrl = url;  // NOLINT(performance-unnecessary-copy-initialization)
                         // because of the ifdef below this can't be const &

#ifdef Q_OS_ANDROID
    realUrl = QUrl(
        QtAndroid::androidActivity()
            .callObjectMethod(
                "contentUrlToFd", "(Ljava/lang/String;)Ljava/lang/String;",
                QAndroidJniObject::fromString(url.toString()).object<jstring>())
            .toString());
#endif

    const QString path =
        realUrl.isLocalFile() ? realUrl.toLocalFile() : QStringLiteral("-");

    const Okular::Document::OpenResult res = m_document->openDocument(
        path, realUrl, db.mimeTypeForUrl(realUrl), password);
    ...
    QString message;
    std::tie(messageType, message) =
        SignatureGuiUtils::documentSignatureMessageWidgetText(m_document);
    if(!message.isEmpty())
    {
...
QString DocumentItem::windowTitleForDocument() const
{
    ...
    QString title =
        Okular::Settings::displayDocumentNameOrPath() ==
                Okular::Settings::EnumDisplayDocumentNameOrPath::Path
            ? m_document->currentDocument().toDisplayString(
                  QUrl::PreferLocalFile)
            : m_document->currentDocument().fileName();

    if(Okular::Settings::displayDocumentTitle())
    {
        const QString docTitle =
            m_document->metaData(QStringLiteral("DocumentTitle")).toString();
...
void DocumentItem::searchText(const QString& text)
{
    ...
    m_document->searchText(PAGEVIEW_SEARCH_ID, text, true, Qt::CaseInsensitive,
                           Okular::Document::AllDocument, true,
                           QColor(100, 100, 200, 40));
...
void DocumentItem::searchFinished(int id,
