...
public:
    ...
    // Should contain all the search terms present in query, includind those
    // from phrases. One element - one term .
    QStringList terms;

    // Should contain phrases present in query without quotes. One element - one
    // phrase.
    QStringList phrases;

    // Should contain all the terms present in all the phrases (but not
    // outside).
...
bool EBookSearch::generateIndex(EBook* ebookFile, QDataStream& stream)
{
    ...
    m_Index = new QtAs::Index();
    connect(m_Index, &QtAs::Index::indexingProgress, this,
            &EBookSearch::updateProgress);

    // Process the list of files in CHM archive and keep only HTML document
    // files from there
...
bool EBookSearch::searchQuery(const QString& query, QList<QUrl>* results,
                              EBook* ebookFile, unsigned int limit)
{
    ...
    for(const QChar& iChar : query)
    {
        ...
        if(splitChars.indexOf(ch) != -1)
        {
            ...
            // Change the term variable, so it will be added when we exit this
            // block
            term = ch;
        }
    ...
    QList<QUrl> foundDocs = m_Index->query(keeper.terms, keeper.phrases,
                                           keeper.phrasewords, ebookFile);

    for(QList<QUrl>::iterator it = foundDocs.begin();
        it != foundDocs.end() && limit > 0; ++it, limit--)
    {
