...
public:
    ...
    //! Associated image number. Used for TOC only; indexes does not have the
    //! image. If IMAGE_NONE, no icon is associated. Otherwise use
    //! getBookIconPixmap() to get associated pixmap icon.
    Icon iconid;
...
public:
     ...
     *
     * Loads a CHM or epub file. For CHM files it could internally load more
     * than one file, if files linked to this one are present locally (like
     * MSDN). \ingroup init
     */
     ...
     */
    virtual bool getFileContentAsString(QString& str,
     ...
     * \ingroup dataretrieve
     */
    virtual bool getFileContentAsBinary(QByteArray& data,
                                        const QUrl& url) const = 0;
     ...
     * array to store list of URLs present in chm archive. \return true if the
     * enumeration succeed; false otherwise (I could hardly imagine a reason).
     *
     * \ingroup dataretrieve
     */
     ...
     * name. Must be implemented, even if the book doesn't support change of
     * encoding (then it should return a default encoding) \return The current
     * encoding.
     *
     * \ingroup encoding
     */
