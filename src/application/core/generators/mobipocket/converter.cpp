...
QTextDocument* Converter::convert(const QString& fileName)
{
    ...
    {
        Q_EMIT error(i18n("This book is protected by DRM and can be displayed "
                          "only on designated device"),
                     -1);
        delete newDocument;
        return nullptr;
    }
    ...
    for(QTextBlock it = newDocument->begin(); it != newDocument->end();
        it = it.next())
    {
        for(QTextBlock::iterator fit = it.begin(); !fit.atEnd(); ++fit)
        {
            ...
            {
                links[format.anchorHref()] = QPair<int, int>(
                    frag.position(), frag.position() + frag.length());
            }
    ...
    while(it.hasNext())
    {
        ...
        if(!u.isRelative())
        {
            Q_EMIT addAction(new Okular::BrowseAction(QUrl(it.key())),
        ...
        else
        {
            ...
            Q_EMIT addAction(
                new Okular::GotoAction(
                    QString(),
                    calculateViewport(newDocument, targets[it.key()])),
                it.value().first, it.value().second);
        }
    }
