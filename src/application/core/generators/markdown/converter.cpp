...
QTextDocument* Converter::convert(const QString& fileName)
{
    ...
    for(auto linkIt = internalLinks.constBegin();
        linkIt != internalLinks.constEnd(); ++linkIt)
    {
        auto anchorIt = documentAnchors.constFind(linkIt.key());
        if(anchorIt != documentAnchors.constEnd())
        {
            const Okular::DocumentViewport viewport =
                calculateViewport(doc, anchorIt.value());
            Okular::GotoAction* action =
                new Okular::GotoAction(QString(), viewport);
            Q_EMIT addAction(
                action, linkIt.value().position(),
                linkIt.value().position() + linkIt.value().length());
        }
        else
        {
...
QTextDocument* Converter::convertOpenFile()
{
    ...
    int flags =
        MKD_FENCEDCODE | MKD_GITHUBTAGS | MKD_AUTOLINK | MKD_TOC | MKD_IDANCHOR;
    ...
    for(QTextBlock::iterator it = parent.begin(); !it.atEnd(); ++it)
    {
        ...
        if(textFragment.isValid())
        {
            ...
            if(textCharFormat.isAnchor())
            {
                ...
                {  // It's an internal link, store it and we'll resolve it at
                   // the end
                    internalLinks.insert(href.mid(1), textFragment);
                }
                else
                {
                    Okular::BrowseAction* action = new Okular::BrowseAction(
                        QUrl(textCharFormat.anchorHref()));
                    Q_EMIT addAction(
                        action, textFragment.position(),
                        textFragment.position() + textFragment.length());
                }
...
void Converter::convertImages(QTextFrame* parent, const QDir& dir,
...
void Converter::convertImages(const QTextBlock& parent, const QDir& dir,
                              QTextDocument* textDocument)
{
    for(QTextBlock::iterator it = parent.begin(); !it.atEnd(); ++it)
    {
        ...
        if(textFragment.isValid())
        {
            ...
            if(textCharFormat.isImageFormat())
            {
                ...
                const qreal specifiedHeight =
                    textCharFormat.toImageFormat().height();
                const qreal specifiedWidth =
                    textCharFormat.toImageFormat().width();
                ...
                const QString imageFilePath = QDir::cleanPath(
                    dir.absoluteFilePath(QUrl::fromPercentEncoding(
                        textCharFormat.toImageFormat().name().toUtf8())));
                ...
                if(QFile::exists(imageFilePath))
                {
                    ...
                    setImageSize(format, specifiedWidth, specifiedHeight,
                                 img.width(), img.height());

                    cursor.insertImage(format);
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
                }
                else if((!textCharFormat.toImageFormat()
                              .property(QTextFormat::ImageAltText)
                              .toString()
                              .isEmpty()))
                {
                    cursor.insertText(textCharFormat.toImageFormat()
                                          .property(QTextFormat::ImageAltText)
                                          .toString());
#endif
                }
...
void Converter::setImageSize(QTextImageFormat& format,
                             const qreal specifiedWidth,
                             const qreal specifiedHeight,
                             const qreal originalWidth,
