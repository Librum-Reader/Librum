...
QTextDocument* Converter::convert(const QString& fileName)
{
    ...
    if(mTitleInfo)
    {
        ...
        {
            Q_EMIT addMetaData(Okular::DocumentInfo::Author,
                               mTitleInfo->mAuthor);
        }

        if(!mTitleInfo->mKeywords.isEmpty())
        {
            Q_EMIT addMetaData(Okular::DocumentInfo::Keywords,
                               mTitleInfo->mKeywords);
        }
    }
    ...
    if(mDocumentInfo)
    {
        ...
        {
            Q_EMIT addMetaData(Okular::DocumentInfo::Producer,
                               mDocumentInfo->mProducer);
        }
    ...
    while(it.hasNext())
    {
        ...
        if(!block.isValid())
        {  // local link without existing target
            continue;
        }
...
bool Converter::convertTitleInfo(const QDomElement& element)
{
    ...
    while(!child.isNull())
    {
        ...
        else if(child.tagName() == QLatin1String("author"))
        {
            ...
            if(!convertAuthor(child, firstName, middleName, lastName, dummy,
                              dummy))
            {
                return false;
            }
            ...
            if(mTitleInfo->mAuthor.isEmpty())
            {
                mTitleInfo->mAuthor = QStringLiteral("%1 %2 %3")
                                          .arg(firstName, middleName, lastName)
                                          .simplified();
            }
            else
            {
...
bool Converter::convertDocumentInfo(const QDomElement& element)
{
    ...
    while(!child.isNull())
    {
        if(child.tagName() == QLatin1String("author"))
        {
            ...
            if(!convertAuthor(child, firstName, middleName, lastName, email,
                              nickname))
            {
                return false;
            }

            mDocumentInfo->mAuthor =
...
bool Converter::convertAuthor(const QDomElement& element, QString& firstName,
                              QString& middleName, QString& lastName,
...
bool Converter::convertDate(const QDomElement& element, QDate& date)
{
    ...
    {
        date = QDate::fromString(element.attribute(QStringLiteral("value")),
                                 Qt::ISODate);
    }
...
bool Converter::convertSection(const QDomElement& element)
{
    ...
    {
        mSectionMap.insert(element.attribute(QStringLiteral("id")),
                           mCursor->block());
    }
...
bool Converter::convertBinary(const QDomElement& element)
{
    ...
    mTextDocument->addResource(QTextDocument::ImageResource, QUrl(id),
...
{
    QString href = element.attributeNS(
        QStringLiteral("http://www.w3.org/1999/xlink"), QStringLiteral("href"));
    ...
    const QImage img = qvariant_cast<QImage>(
        mTextDocument->resource(QTextDocument::ImageResource, QUrl(href)));
...
bool Converter::convertLink(const QDomElement& element)
{
    QString href = element.attributeNS(
        QStringLiteral("http://www.w3.org/1999/xlink"), QStringLiteral("href"));
    QString type = element.attributeNS(
        QStringLiteral("http://www.w3.org/1999/xlink"), QStringLiteral("type"));
    ...
    if(href.startsWith(QLatin1Char('#')))
    {  // local link
        mLocalLinks.insert(href.mid(1),
                           QPair<int, int>(startPosition, endPosition));
    }
    else
    {
        // external link
...
bool Converter::convertTableHeaderCell(const QDomElement& element,
                                       QTextTable& table, int& column)
{
    QTextCharFormat charFormat;
    charFormat.setFontWeight(QFont::Bold);
...
bool Converter::convertTableCell(const QDomElement& element, QTextTable& table,
                                 int& column)
{
    QTextCharFormat charFormat;
    return convertTableCellHelper(element, table, column, charFormat);
...
bool Converter::convertTableCellHelper(const QDomElement& element,
                                       QTextTable& table, int& column,
                                       const QTextCharFormat& charFormat)
{
    ...
    // TODO: rowspan
    // int rowspan = qMax(element.attribute(QStringLiteral("rowspan")).toInt(),
    // 1);
