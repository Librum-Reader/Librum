...
static void imagesInArchive(const QString& prefix, const KArchiveDirectory* dir,
                            QStringList* entries)
{
    ...
    for(const QString& file : entryList)
    {
        ...
        if(e->isDirectory())
        {
            imagesInArchive(prefix + file + QLatin1Char('/'),
                            static_cast<const KArchiveDirectory*>(e), entries);
        }
        else if(e->isFile())
        {
...
bool Document::open(const QString& fileName)
{
    ...
    else if(mime.inherits(QStringLiteral("application/x-cbt")) ||
            mime.inherits(QStringLiteral("application/x-gzip")) ||
    ...
    else if(mime.inherits(QStringLiteral("application/x-cb7")) ||
            ...
            mime.inherits(QStringLiteral("application/vnd.rar")))
    {
        ...
        {
            mLastErrorString = i18n("Cannot open document, neither unrar nor "
                                    "unarchiver were found.");
            return false;
        }

        if(!Unrar::isSuitableVersionAvailable())
        {
            mLastErrorString = i18n("The version of unrar on your system is "
...
void Document::pages(QVector<Okular::Page*>* pagesVector)
{
    std::sort(mEntries.begin(), mEntries.end(),
              caseSensitiveNaturalOrderLessThen);
    ...
    for(const QString& file : qAsConst(mEntries))
    {
        if(mArchive)
        {
            const KArchiveFile* entry =
                static_cast<const KArchiveFile*>(mArchiveDir->entry(file));
            if(entry)
            {
        ...
        if(!dev.isNull())
        {
            ...
            if(reader.canRead())
            {
                ...
                if(pageSize.isValid())
                {
                    pagesVector->replace(
                        count,
                        new Okular::Page(count, pageSize.width(),
                ...
                else
                {
                    qCDebug(OkularComicbookDebug)
                        << "Ignoring" << file
                        << "doesn't seem to be an image even if "
...
QImage Document::pageImage(int page) const
{
    if(mArchive)
    {
        const KArchiveFile* entry = static_cast<const KArchiveFile*>(
            mArchiveDir->entry(mPageMap[page]));
        if(entry)
        {
            std::unique_ptr<QIODevice> dev(entry->createDevice());
            // This could simply be
            //     QImageReader reader(dev.get());
            // but due to https://codereview.qt-project.org/c/qt/qtbase/+/349174
            // and
            // https://invent.kde.org/frameworks/karchive/-/merge_requests/14 it
            // can not, so it will have to be like this at least until Qt6 Test
            // with https://bugs.kde.org/attachment.cgi?id=74039 (it's a cbz
            // with a png inside)
            QBuffer b;
