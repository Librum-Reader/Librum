...
static void which_ddjvu_message(const ddjvu_message_t* msg)
{
    ...
    {
    case DDJVU_ERROR:
        qDebug().nospace() << "ERROR: file " << msg->m_error.filename
                           << ", line " << msg->m_error.lineno;
 ...
 */
static void wait_for_ddjvu_message(ddjvu_context_t* ctx,
...
static bool find_replace_or_add_second_in_pair(miniexp_t theexp,
                                               const char* which,
...
void KDjVu::TextAnnotation::setColor(const QColor& color)
{
    ...
    find_replace_or_add_second_in_pair(m_anno, "backclr",
                                       miniexp_symbol(col.constData()));
}
...
{
    const QByteArray col = color.name().toLatin1();
    find_replace_or_add_second_in_pair(m_anno, "lineclr",
                                       miniexp_symbol(col.constData()));
}
...
public:
    ...
    QImage generateImageTile(ddjvu_page_t* djvupage, int& res, int width,
                             int row, int xdelta, int height, int col,
                             int ydelta);
...
unsigned int KDjVu::Private::s_formatmask[4] = { 0x00ff0000, 0x0000ff00,
                                                 0x000000ff, 0xff000000 };

QImage KDjVu::Private::generateImageTile(ddjvu_page_t* djvupage, int& res,
                                         int width, int row, int xdelta,
                                         int height, int col, int ydelta)
{
    ...
    res = ddjvu_page_render(djvupage, DDJVU_RENDER_COLOR, &pagerect,
                            &renderrect, m_format, res_img.bytesPerLine(),
                            (char*)res_img.bits());
    if(!res)
    {
...
void KDjVu::Private::readBookmarks()
{
    ...
    miniexp_t outline;
    while((outline = ddjvu_document_get_outline(m_djvu_document)) ==
          miniexp_dummy)
    {
        handle_ddjvu_messages(m_djvu_cxt, true);
    }

    if(miniexp_listp(outline) && (miniexp_length(outline) > 0) &&
       miniexp_symbolp(miniexp_nth(0, outline)) &&
       (QString::fromUtf8(miniexp_to_name(miniexp_nth(0, outline))) ==
...
void KDjVu::Private::fillBookmarksRecurse(QDomDocument& maindoc,
                                          QDomNode& curnode, miniexp_t exp,
                                          int offset)
{
           ...
           miniexp_stringp(miniexp_nth(1, cur)))
        {
            ...
            if(!dest.isEmpty())
            {
                if(dest.at(0) == QLatin1Char('#'))
                {
                    ...
                    if(isNumber)
                    {
                        // it might be an actual page number, but could also be
                        // a page label so resolve the number, and get the real
                        // page number
                        int pageNo = pageWithName(dest);
                        if(pageNo != -1)
                        {
                            el.setAttribute(QStringLiteral("PageNumber"),
                                            QString::number(pageNo + 1));
                        }
                        else
                        {
...
void KDjVu::Private::readMetaData(int page)
{
    ...
    miniexp_t annots;
    while((annots = ddjvu_document_get_pageanno(m_djvu_document, page)) ==
          miniexp_dummy)
    {
        handle_ddjvu_messages(m_djvu_cxt, true);
    }
...
int KDjVu::Private::pageWithName(const QString& name)
{
    ...
    {
        if(DDJVU_JOB_OK !=
           ddjvu_document_get_fileinfo(m_djvu_document, i, &info))
        {
            continue;
        }
        if(info.type != 'P')
        ...
        if((utfName == info.id) || (utfName == info.name) ||
           (utfName == info.title))
        {
...
bool KDjVu::openFile(const QString& fileName)
{
    ...
    // load the document..., use UTF-8 variant to work on Windows, too, see bug
    // 422500
    d->m_djvu_document = ddjvu_document_create_by_filename_utf8(
        d->m_djvu_cxt, fileName.toUtf8().constData(), true);
    if(!d->m_djvu_document)
    {
        return false;
    }
    ...
    d->m_metaData[QStringLiteral("componentFile")] =
        ddjvu_document_get_filenum(d->m_djvu_document);
    ...
    for(int i = 0; i < numofpages; ++i)
    {
        ...
        while((sts = ddjvu_document_get_pageinfo(d->m_djvu_document, i,
                                                 &info)) < DDJVU_JOB_OK)
        {
...
void KDjVu::linksAndAnnotationsForPage(
    ...
    QList<KDjVu::Annotation*>* annotations) const
{
    ...
    miniexp_t annots;
    while((annots = ddjvu_document_get_pageanno(d->m_djvu_document, pageNum)) ==
          miniexp_dummy)
    {
        handle_ddjvu_messages(d->m_djvu_cxt, true);
    }
    ...
    for(int i = 0; i < l; ++i)
    {
        ...
        if(miniexp_symbolp(miniexp_nth(0, miniexp_nth(3, cur))))
        {
            type = QString::fromUtf8(
                miniexp_to_name(miniexp_nth(0, miniexp_nth(3, cur))));
        }
        KDjVu::Link* link = nullptr;
        KDjVu::Annotation* ann = nullptr;
        miniexp_t urlexp = miniexp_nth(1, cur);
        if(links &&
           (type == QLatin1String("rect") || type == QLatin1String("oval") ||
            type == QLatin1String("poly")))
        {
            if(miniexp_stringp(urlexp))
            {
                target = QString::fromUtf8(miniexp_to_str(miniexp_nth(1, cur)));
                    ...
                    (qstrncmp(miniexp_to_name(miniexp_nth(0, urlexp)), "url",
                              3) == 0))
            {
                target =
            ...
            if(target.isEmpty() ||
               ((target.length() > 0) && target.at(0) == QLatin1Char('#')))
            {
        ...
        else if(annotations && (type == QLatin1String("text") ||
        ...
        if(link /* safety check */ && links)
        {
            ...
            if((arealength == 5) &&
               (type == QLatin1String("rect") || type == QLatin1String("oval")))
            {
                link->m_point = QPoint(miniexp_to_int(miniexp_nth(1, area)),
                                       miniexp_to_int(miniexp_nth(2, area)));
                link->m_size = QSize(miniexp_to_int(miniexp_nth(3, area)),
                    ...
                    type == QLatin1String("poly"))
            {
                ...
                {
                    poly << QPoint(miniexp_to_int(miniexp_nth(j, area)),
                                   miniexp_to_int(miniexp_nth(j + 1, area)));
                }
                link->m_poly = poly;
            }
...
QImage KDjVu::image(int page, int width, int height, int rotation)
{
    ...
    {
        ddjvu_page_t* newpage =
            ddjvu_page_create_by_pageno(d->m_djvu_document, page);
    ...
    //        ddjvu_page_set_rotation( djvupage, m_pages.at( page
    )->orientation() ); ddjvu_page_set_rotation( djvupage,
    (ddjvu_page_rotation_t)flipRotation( rotation ) );
        }
    */
    ...
    if((xparts == 1) && (yparts == 1))
    {
        ...
        newimg = d->generateImageTile(djvupage, res, width, 0, xdelta, height,
                                      0, ydelta);
    }
    else
    {
        ...
        for(int i = 0; i < parts; ++i)
        {
            ...
            int tmpres = 0;
            const QImage tempp = d->generateImageTile(
                djvupage, tmpres, width, row, xdelta, height, col, ydelta);
    ...
    if(res && d->m_cacheEnabled)
    {
        ...
        if(imgsize > 0)
        {
            for(int i = 0; i < d->mImgCache.count();)
            {
                ...
                if((cur->page == page) &&
                   (abs(cur->img.width() * cur->img.height() - imgsize) <
                    imgsize * 0.35))
                {
...
bool KDjVu::exportAsPostScript(const QString& fileName,
                               const QList<int>& pageList) const
{
    if(!d->m_djvu_document || fileName.trimmed().isEmpty() ||
       pageList.isEmpty())
    {
        return false;
    }
...
bool KDjVu::exportAsPostScript(QFile* file, const QList<int>& pageList) const
{
    ...
    ddjvu_job_t* printjob =
        ddjvu_document_print(d->m_djvu_document, f, optc, optv);
...
QList<KDjVu::TextEntity> KDjVu::textEntities(int page,
                                             const QString& granularity) const
{
    ...
    miniexp_t r;
    while((r = ddjvu_document_get_pagetext(d->m_djvu_document, page,
                                           nullptr)) == miniexp_dummy)
    {
        handle_ddjvu_messages(d->m_djvu_cxt, true);
    }
