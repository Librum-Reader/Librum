...
static int ReadConfigFile(const char* filename)
{
    ...
    while(true)
    {
        ...
        {
            if((str_end = strchr(linebuf + buf_index + 1, SEGMENT_END_CHAR)) ==
               nullptr)
            {
                /* invalid segment line */
                _plkr_message("%s:%d:  Invalid segment line '%s'", filename,
                              line_number, linebuf);
                goto error_exit;
            }
                ...
                current_option != nullptr)
        {
            /* continuation line */
            str_begin =
                (char*)_plkr_RemoveFromTable(current_segment, current_option);
            for(str_end = linebuf + strlen(linebuf) - 1;
                str_end > linebuf && isspace(*str_end); str_end--)
            {
                ;
            }
            charptr = (char*)malloc(strlen(str_begin) +
        ...
        else if((int)strcspn(linebuf, OPTION_SEPARATOR_CHARS) < linelen)
        {
            ...
            if(ptr == str_begin)
            {
                _plkr_message("%s:%d:  Invalid option line '%s'", filename,
                              line_number, linebuf);
                goto error_exit;
            }
                           ...
                           char* default_value)
{
    ...
    {
        if((section = (HashTable*)_plkr_FindInTable(SectionsTable,
    ...
    if(value == nullptr &&
       ((section_name == nullptr) || (strcmp(section_name, "default") != 0)))
    {
        if((section = (HashTable*)_plkr_FindInTable(
                SectionsTable, STRINGIFY(OS_SECTION_NAME))) != nullptr)
        {
            value = (char*)_plkr_FindInTable(section, option_name);
        }
    }
                           ...
                           long int default_value)
{
    ...
    if(*endptr != 0)
    {
        _plkr_message("Bad int value string '%s' for option %s:%s", svalue,
                      (section_name ? section_name : "default"), option_name);
        return default_value;
    }
    else
    {
                           ...
                           double default_value)
{
    ...
    if(*endptr != 0)
    {
        _plkr_message("Bad float value string '%s' for option %s:%s", svalue,
                      (section_name ? section_name : "default"), option_name);
        return default_value;
    }
    else
                          ...
                          int default_value)
{
    ...
    if((strcmp(svalue, "1") == 0) || (strcmp(svalue, "true") == 0) ||
       (strcmp(svalue, "TRUE") == 0) || (strcmp(svalue, "on") == 0) ||
       (strcmp(svalue, "ON") == 0) || (strcmp(svalue, "t") == 0) ||
       (strcmp(svalue, "T") == 0) || (strcmp(svalue, "True") == 0))
    {
        return 1;
    }
    else if((strcmp(svalue, "0") == 0) || (strcmp(svalue, "false") == 0) ||
            (strcmp(svalue, "FALSE") == 0) || (strcmp(svalue, "off") == 0) ||
            (strcmp(svalue, "OFF") == 0) || (strcmp(svalue, "F") == 0) ||
            (strcmp(svalue, "f") == 0) || (strcmp(svalue, "False") == 0))
    {
        return 0;
    }
    else
    {
        _plkr_message("Bad boolean value string '%s' for option %s:%s", svalue,
                      (section_name ? section_name : "default"), option_name);
        return default_value;
    }
}
