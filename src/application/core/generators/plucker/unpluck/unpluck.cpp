...
static plkr_DataRecord* FindRecordByIndex(plkr_Document* doc, int record_index)
{
    ...
    for(imin = 0, imax = doc->nrecords; imin < imax;)
    {
        ...
        /* _plkr_message("imin = %2d, imax = %2d, itest = %2d (%2d),
           record_index = %2d", imin, imax, itest, doc->records[itest].uid,
           record_index); */
...
static int GetUncompressedRecord(plkr_Document* doc, plkr_DBHandle handle,
                                 int record_index, unsigned char* buffer,
                                 int buffer_size,
...
{
    /* read whole data record, including header, into buffer.  If some part of
       the record is compressed, uncompress it.  If "buffer" is NULL, allocate
       enough bytes to fit.  Returns TRUE if read is successful, and sets
       "buffer_out" and "buffer_size_out" and "record_out" on successful return.
     */
           ...
           (record->type == PLKR_DRTYPE_LINKS_COMPRESSED))
        {
            ...
            if(!handle->seek(handle, record->offset) ||
               (handle->read(handle, buf, record->size, record->size) !=
                record->size))
            {
                _plkr_message("Bad read from DBHandle while reading record %d",
                              record->uid);
                free(buf);
                if(tbuffer != buffer)
                {
            ...
            if(doc->compression == PLKR_COMPRESSION_ZLIB)
            {
                if(UncompressZLib(start_of_data, len_of_data, output_ptr,
                                  buf_to_use,
                                  (doc->owner_id_required ? doc->owner_id_key
                                                          : nullptr)) != Z_OK)
                {
...
static int ParseCategories(plkr_Document* newdoc, plkr_DBHandle handle)
{
    ...
    if(GetUncompressedRecord(newdoc, handle,
                             newdoc->default_category_record_uid, nullptr, 0,
                             PLKR_DRTYPE_CATEGORY, &buf, &bufsize, &record))
    {
        /* keep the record data, since the list of char * ptrs will point into
         * it */
        record->cache = buf;
        ...
        {
            newc = (struct _plkr_CategoryName*)malloc(
                sizeof(struct _plkr_CategoryName));
...
static int ParseMetadata(plkr_Document* newdoc, plkr_DBHandle handle)
{
    ...
    if(!GetUncompressedRecord(newdoc, handle, newdoc->metadata_record_uid,
                              nullptr, 0, PLKR_DRTYPE_METADATA, &buf, &bufsize,
                              nullptr))
    {
        return FALSE;
    }
    else
    {
        ...
        for(i = 0, ptr = buf + 10; i < nsubrecords; i++)
        {
            ...
            {
                newdoc->author =
                    _plkr_strndup((char*)(ptr + 4), subrecord_length);
...
static int ParseURLs(plkr_Document* newdoc, plkr_DBHandle handle)
{
    ...
    if(!GetUncompressedRecord(newdoc, handle, newdoc->urls_index_record_uid,
                              nullptr, 0, PLKR_DRTYPE_LINKS_INDEX, &buf,
                              &bufsize, nullptr))
    {
        return FALSE;
    }
    else
    {
        ...
        for(i = 0, nurls = 0; i < n; i++)
        {
            ...
            _plkr_message("index %3d:  last = %d, record_id = %d", i,
                          records[i].last_url_index, records[i].record_id);
#endif /* def DEBUGURLS */
    ...
    for(count = 0, i = 0; i < n; i++)
    {
        ...
        if(!GetUncompressedRecord(newdoc, handle, id, nullptr, 0,
                                  PLKR_DRTYPE_NONE, &buf, &bufsize, &record))
        {
            goto errout4;
        }
...
plkr_Document* plkr_OpenDoc(plkr_DBHandle handle)
{
    ...
    newdoc->name =
        (char*)_plkr_strndup((char*)utilbuf, MIN(strlen((char*)utilbuf), 32));
    newdoc->creation_time =
        (time_t)((utilbuf[36] << 24) + (utilbuf[37] << 16) +
                 (utilbuf[38] << 8) + utilbuf[39] - PLKR_TIMEADJUST);
    newdoc->modification_time =
        (time_t)((utilbuf[40] << 24) + (utilbuf[41] << 16) +
                 (utilbuf[42] << 8) + utilbuf[43] - PLKR_TIMEADJUST);
    ...
    for(i = 0; i < newdoc->nrecords; i++)
    {
        ...
        newdoc->records[i].offset = (utilbuf[0] << 24) + (utilbuf[1] << 16) +
                                    (utilbuf[2] << 8) + utilbuf[3];
    }
    ...
    /* To get the size of the last record we subtract its offset from the total
     * size of the DB. */
...
#ifdef DEBUGOPEN
        {
            static char* types[] = { "TEXT",       "TEXTC",   "IMAGE",
                                     "IMAGEC",     "MAILTO",  "URLINDEX",
                                     "URLS",       "URLSC",   "BOOKMARKS",
                                     "CATEGORIES", "METADATA" };
            _plkr_message(
                "%3d:  type=%10s, offset=%07x, size=%5d, uncompressed_size=%5d",
                record->uid,
                types[MIN(record->type, sizeof(types) / sizeof(char*))],
    ...
    if(newdoc->owner_id_required)
    {
        ...
        if(owner_id != nullptr)
        {
            ...
            {
                crc = crc32(crc, (const Bytef*)owner_id, owner_id_len);
                newdoc->owner_id_key[(i * 4) + 0] =
                    (unsigned char)((crc >> 24) & 0xFF);
                newdoc->owner_id_key[(i * 4) + 1] =
                    (unsigned char)((crc >> 16) & 0xFF);
                newdoc->owner_id_key[(i * 4) + 2] =
    ...
    for(i = 1; i < newdoc->nrecords; i++)
    {
        ...
        printf("==============================================\n"
               "record %3d (%d bytes)\n",
               newdoc->records[i].uid, newdoc->records[i].size);
...
static int FpRead(plkr_DBHandle handle, unsigned char* buffer, int buffersize,
                  int readsize)
{
    int result;
                         ...
                         plkr_DataRecordType* type)
{
    ...
    if(!GetUncompressedRecord(doc, doc->handle, record_index, output_buffer,
                              output_buffer_size, PLKR_DRTYPE_NONE, nullptr,
                              &output_size, &record))
    {
        return 0;
    }
...
unsigned char* plkr_GetRecordBytes(plkr_Document* doc, int record_index,
                                   int* size, plkr_DataRecordType* type)
{
    plkr_DataRecord* record;
    unsigned char* buf;
    ...
    if(!GetUncompressedRecord(doc, doc->handle, record_index, nullptr, 0,
                              PLKR_DRTYPE_NONE, &buf, size, &record))
    {
        return nullptr;
    }
    else
