...
static bool parseGUID(const QString& guidString, unsigned short guid[16])
{
    ...
    const static int indexes[] = { 6,  4,  2,  0,  11, 9,  16, 14,
...
static bool nextAbbPathToken(AbbPathToken* token)
{
    ...
    if(ch.isNumber() || (ch == QLatin1Char('+')) || (ch == QLatin1Char('-')))
    {
        int start = *curPos;
        while((*curPos < data.length()) && (!data.at(*curPos).isSpace()) &&
              (data.at(*curPos) != QLatin1Char(',') &&
               (!data.at(*curPos).isLetter() ||
                data.at(*curPos) == QLatin1Char('e'))))
        {
            (*curPos)++;
        }
...
*/
static QPointF getPointFromString(AbbPathToken* token, bool relative,
...
static QPointF getPointFromString(const QString& string)
{
    ...
    if(commaPos == -1 ||
       string.indexOf(QLatin1Char(QLatin1Char(',')), commaPos + 1) != -1)
    {
        return QPointF();
    }
    ...
    result.setX(
        QString::fromRawData(ref.constData(), ref.count()).toDouble(&ok));
    if(!ok)
    {
        return QPointF();
    }
...
static Qt::FillRule fillRuleFromString(const QString& data,
                                       Qt::FillRule def = Qt::OddEvenFill)
{
...
static QPainterPath parseAbbreviatedPathData(const QString& data)
{
    ...
    while(true)
    {
        if(token.type != abtCommand)
        {
            ...
            {
                qCWarning(OkularXpsDebug).nospace()
                    << "Error in parsing abbreviated path data (" << token.type
                    << "@" << token.curPos << "): " << data;
            }
            return path;
        }
        ...
        case 'f':
            ...
            {
                // In xps specs rule 1 means NonZero fill. I think it's
                // equivalent to WindingFill but I'm not sure
        ...
        case 'c':  // Cubic bezier curve
            while(token.type == abtNumber)
            {
                QPointF firstControl =
                    getPointFromString(&token, isRelative, currPos);
                QPointF secondControl =
                    getPointFromString(&token, isRelative, currPos);
                QPointF endPoint =
                    getPointFromString(&token, isRelative, currPos);
        ...
        case 's':  // Smooth cubic bezier curve
            while(token.type == abtNumber)
            {
                ...
                {
                    firstControl =
                        lastSecondControlPoint +
                        (lastSecondControlPoint + path.currentPosition());
                }
                else
                {
...
/**
   \return The absolute path of the \p location, according to \p path if it's
   non-absolute
*/
...
*/
static QByteArray readFileOrDirectoryParts(const KArchiveEntry* entry,
                                           QString* pathOfFile = nullptr)
{
    QByteArray data;
    if(entry->isDirectory())
    {
        const KArchiveDirectory* relDir =
        ...
        for(const QString& entry : qAsConst(entries))
        {
            ...
            const KZipFileEntry* relSubFile =
                static_cast<const KZipFileEntry*>(relSubEntry);
...
/**
   Load the resource \p fileName from the specified \p archive using the case
   sensitivity \p cs
*/
static const KArchiveEntry* loadEntry(KZip* archive, const QString& fileName,
                                      Qt::CaseSensitivity cs)
{
    // first attempt: loading the entry straight as requested
    const KArchiveEntry* entry = archive->directory()->entry(fileName);
    // in case sensitive mode, or if we actually found something, return what we
    // found
    ...
    {
        const KArchiveDirectory* relDir =
            static_cast<const KArchiveDirectory*>(newEntry);
...
static int xpsGradientWithOffset(const QList<XpsGradient>& gradients,
                                 double offset)
{
    int i = 0;
...
static void preprocessXpsGradients(QList<XpsGradient>& gradients)
{
    ...
    if(xpsGradientWithOffset(gradients, 0.0) == -1)
    {
        ...
        while(firstGreaterThanZero < gradients.count() &&
              gradients.at(firstGreaterThanZero).offset < 0.0)
        {
    ...
    if(xpsGradientWithOffset(gradients, 1.0) == -1)
    {
        ...
        while(firstLessThanOne >= 0 &&
              gradients.at(firstLessThanOne).offset > 1.0)
        {
            --firstLessThanOne;
        }
...
static void addXpsGradientsToQGradient(const QList<XpsGradient>& gradients,
                                       QGradient* qgrad)
{
    for(const XpsGradient& grad : gradients)
    {
...
bool XpsHandler::endElement(const QString& nameSpace, const QString& localName,
                            const QString& qname)
{
    Q_UNUSED(nameSpace)
    Q_UNUSED(qname)
...
{
    // TODO Currently ignored attributes: CaretStops, DeviceFontName,
    // IsSideways, OpacityMask, Name, FixedPage.NavigateURI, xml:lang, x:key
    ...
    // Get font (doesn't work well because qt doesn't allow to load font from
    // file) This works despite the fact that font size isn't specified in
    // points as required by qt. It's because I set point size to be equal to
    // drawing unit.
    float fontSize =
        node.attributes.value(QStringLiteral("FontRenderingEmSize")).toFloat();
    // qCWarning(OkularXpsDebug) << "Font Rendering EmSize:" << fontSize;
    // a value of 0.0 means the text is not visible (see XPS specs, chapter 12,
    // "Glyphs")
    if(fontSize < 0.1)
    {
        m_painter->restore();
    ...
    const QString absoluteFileName =
        absolutePath(entryPath(m_page->fileName()),
                     node.attributes.value(QStringLiteral("FontUri")));
    ...
    // Origin
    QPointF origin(node.attributes.value(QStringLiteral("OriginX")).toDouble(),
                   node.attributes.value(QStringLiteral("OriginY")).toDouble());
    ...
    else
    {
        ...
        if(brush.style() > Qt::NoBrush &&
           brush.style() < Qt::LinearGradientPattern &&
           brush.color().alpha() == 0)
        {
            m_painter->restore();
            return;
    ...
    if(!att.isEmpty())
    {
        ...
        for(int i = 0; i < indicesElements.size(); ++i)
        {
            ...
            {
                QStringList parts =
                    indicesElements.at(i).split(QLatin1Char(','));
                if(parts.size() == 2)
                {
                    // regular advance case, no offsets
                    advanceWidths.append(parts.at(1).toDouble() * fontSize /
                                         100.0);
                }
                else if(parts.size() == 3)
                {
                    ...
                    qreal AdvanceWidth =
                        parts.at(1).toDouble() * fontSize / 100.0;
                    qreal uOffset = parts.at(2).toDouble() / 100.0;
                    advanceWidths.append(AdvanceWidth + uOffset);
                }
                else
                {
                    // has vertical offset, but don't know how to handle that
                    // yet
                    qCWarning(OkularXpsDebug) << "Unhandled Indices element: "
                                              << indicesElements.at(i);
    ...
    // UnicodeString
    QString stringToDraw(
        unicodeString(node.attributes.value(QStringLiteral("UnicodeString"))));
    ...
    // qCWarning(OkularXpsDebug) << "Glyphs: " << atts.value("Fill") << ", " <<
    // atts.value("FontUri"); qCWarning(OkularXpsDebug) << "    Origin: " <<
    // atts.value("OriginX") << "," << atts.value("OriginY");
    // qCWarning(OkularXpsDebug) << "    Unicode: " <<
    // atts.value("UnicodeString");
...
void XpsHandler::processFill(XpsRenderNode& node)
{
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Fill element should have exactly one child";
    }
    else
    {
...
void XpsHandler::processStroke(XpsRenderNode& node)
{
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Stroke element should have exactly one child";
    }
    else
    {
        node.data = node.children[0].data;
...
{
    // TODO Ignored attributes: Opacity, x:key, TileMode, ViewBoxUnits,
    // ViewPortUnits
    // TODO Check whether transformation works for non standard situations
    // (viewbox different that whole image, Transform different that simple move
    // & scale, Viewport different than [0, 0, 1, 1]

    QString att;
    QBrush brush;

    QRectF viewport =
        stringToRectF(node.attributes.value(QStringLiteral("Viewport")));
    QRectF viewbox =
        stringToRectF(node.attributes.value(QStringLiteral("Viewbox")));
    QImage image = m_page->loadImageFromFile(
        node.attributes.value(QStringLiteral("ImageSource")));

    // Matrix which can transform [0, 0, 1, 1] rectangle to given viewbox
    QTransform viewboxMatrix = QTransform(
        viewbox.width() * image.physicalDpiX() / 96, 0, 0,
        viewbox.height() * image.physicalDpiY() / 96, viewbox.x(), viewbox.y());
    ...
    {
        QVariant data =
            node.getChildData(QStringLiteral("ImageBrush.Transform"));
    ...
    viewportMatrix =
        viewportMatrix * QTransform(viewport.width(), 0, 0, viewport.height(),
...
{
    // TODO Ignored attributes: Clip, OpacityMask, StrokeEndLineCap,
    // StorkeStartLineCap, Name, FixedPage.NavigateURI, xml:lang, x:key,
    ...
    // Get path
    XpsPathGeometry* pathdata = node.getChildData(QStringLiteral("Path.Data"))
                                    .value<XpsPathGeometry*>();
    ...
    if(!att.isEmpty())
    {
        const QStringList pieces =
            att.split(QLatin1Char(' '), QString::SkipEmptyParts);
    ...
    if(!att.isEmpty())
    {
        ...
        if(ok)
        {
            // we have to divide it by two, as XPS consider half of the stroke
            // width, while Qt the whole of it
...
void XpsHandler::processPathData(XpsRenderNode& node)
{
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Path.Data element should have exactly one child";
    }
    else
    {
        node.data = node.children[0].data;
        ...
        else if(child.name == QLatin1String("PolyBezierSegment"))
        {
            ...
            if(!att.isEmpty())
            {
                ...
                if(points.count() % 3 == 0)
                {
                    ...
                    {
                        QPointF firstControl =
                            getPointFromString(points.at(i++));
                        QPointF secondControl =
                            getPointFromString(points.at(i++));
...
void XpsHandler::processEndElement(XpsRenderNode& node)
{
    ...
    {
        // TODO Ignoring x:key
        node.data = QVariant::fromValue(QTransform(
            attsToMatrix(node.attributes.value(QStringLiteral("Matrix")))));
    }
    else if((node.name == QLatin1String("Canvas.RenderTransform")) ||
            (node.name == QLatin1String("Glyphs.RenderTransform")) ||
            (node.name == QLatin1String("Path.RenderTransform")))
    {
        QVariant data =
            node.getRequiredChildData(QStringLiteral("MatrixTransform"));
        if(data.canConvert<QTransform>())
        {
    ...
    else if((node.name == QLatin1String("Path.Fill")) ||
            (node.name == QLatin1String("Glyphs.Fill")))
    {
        processFill(node);
    }
    ...
    else if(node.name == QLatin1String("SolidColorBrush"))
    {
        ...
        node.data = QVariant::fromValue(QBrush(QColor(hexToRgba(
            node.attributes.value(QStringLiteral("Color")).toLatin1()))));
    }
    ...
    {
        const XpsRenderNode* gradients =
            node.findChild(QStringLiteral("LinearGradientBrush.GradientStops"));
        if(gradients && gradients->data.canConvert<QGradient*>())
        {
            QPointF start = getPointFromString(
                node.attributes.value(QStringLiteral("StartPoint")));
            QPointF end = getPointFromString(
                node.attributes.value(QStringLiteral("EndPoint")));
            QLinearGradient* qgrad = static_cast<QLinearGradient*>(
                gradients->data.value<QGradient*>());
            qgrad->setStart(start);
            qgrad->setFinalStop(end);
            applySpreadStyleToQGradient(
                node.attributes.value(QStringLiteral("SpreadMethod")), qgrad);
    ...
    {
        const XpsRenderNode* gradients =
            node.findChild(QStringLiteral("RadialGradientBrush.GradientStops"));
        if(gradients && gradients->data.canConvert<QGradient*>())
        {
            QPointF center = getPointFromString(
                node.attributes.value(QStringLiteral("Center")));
            QPointF origin = getPointFromString(
                node.attributes.value(QStringLiteral("GradientOrigin")));
            double radiusX =
                node.attributes.value(QStringLiteral("RadiusX")).toDouble();
            double radiusY =
                node.attributes.value(QStringLiteral("RadiusY")).toDouble();
            QRadialGradient* qgrad = static_cast<QRadialGradient*>(
                gradients->data.value<QGradient*>());
    ...
    else if(node.name == QLatin1String("LinearGradientBrush.GradientStops"))
    {
        ...
        {
            double offset =
                child.attributes.value(QStringLiteral("Offset")).toDouble();
            QColor color = hexToRgba(
                child.attributes.value(QStringLiteral("Color")).toLatin1());
    ...
    m_pageIsRendered(false)
{
    ...
    const KZipFileEntry* pageFile = static_cast<const KZipFileEntry*>(
        m_file->xpsArchive()->directory()->entry(fileName));
    ...
    while(!xml.atEnd())
    {
        ...
        if(xml.isStartElement() && (xml.name() == QStringLiteral("FixedPage")))
        {
            ...
            m_pageSize.setWidth(attributes.value(QStringLiteral("Width"))
                                    .toString()
                                    .toDouble());
            m_pageSize.setHeight(attributes.value(QStringLiteral("Height"))
                                     .toString()
                                     .toDouble());
            break;
        }
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Could not parse XPS page:" << xml.errorString();
    }
}
...
bool XpsPage::renderToImage(QImage* p)
{
    if((m_pageImage == nullptr) || (m_pageImage->size() != p->size()))
    {
        ...
        // Set one point = one drawing unit. Useful for fonts, because xps
        // specifies font size using drawing units, not points as usual
...
bool XpsPage::renderToPainter(QPainter* painter)
{
    ...
    handler.m_painter->setWorldTransform(QTransform().scale(
        (qreal)painter->device()->width() / size().width(),
        (qreal)painter->device()->height() / size().height()));
    QXmlSimpleReader parser;
    parser.setContentHandler(&handler);
    parser.setErrorHandler(&handler);
    const KZipFileEntry* pageFile = static_cast<const KZipFileEntry*>(
        m_file->xpsArchive()->directory()->entry(m_fileName));
...
{
    // qCWarning(OkularXpsDebug) << "trying to get font: " << fileName << ",
    // size: " << size;
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Requesting unknown font:" << absoluteFileName;
        return QFont();
    }

    const QStringList fontFamilies =
        m_fontDatabase.applicationFontFamilies(index);
    if(fontFamilies.isEmpty())
    {
        qCWarning(OkularXpsDebug)
            << "The unexpected has happened. No font family for a known font:"
            << absoluteFileName << index;
        return QFont();
    }
    ...
    {
        qCWarning(OkularXpsDebug) << "The unexpected has happened. No font "
                                     "style for a known font family:"
                                  << absoluteFileName << index << fontFamily;
        return QFont();
    }
...
int XpsFile::loadFontByName(const QString& absoluteFileName)
{
    ...
    const KArchiveEntry* fontFile =
        loadEntry(m_xpsArchive, absoluteFileName, Qt::CaseInsensitive);
    if(!fontFile)
    {
        return -1;
    }

    QByteArray fontData = readFileOrDirectoryParts(
        fontFile);  // once per file, according to the docs

    int result = m_fontDatabase.addApplicationFontFromData(fontData);
    if(-1 == result)
    {
        // Try to deobfuscate font
        // TODO Use deobfuscation depending on font content type, don't do it
        // always when standard loading fails
        ...
        {
            qCWarning(OkularXpsDebug)
                << "File to load font - file name isn't a GUID";
        }
        else
        {
            ...
            else
            {
                ...
                // (font's filename)
                const static int mapping[] = { 15, 14, 13, 12, 11, 10, 9, 8,
                                               6,  7,  4,  5,  0,  1,  2, 3 };
                for(int i = 0; i < 16; i++)
                {
    ...
    // qCWarning(OkularXpsDebug) << "Loaded font: " <<
    // m_fontDatabase.applicationFontFamilies( result );
...
QImage XpsPage::loadImageFromFile(const QString& fileName)
{
    ...
    {
        // for example: '{ColorConvertedBitmap /Resources/bla.wdp
        // /Resources/foobar.icc}'
        // TODO: properly read a ColorConvertedBitmap
        return QImage();
    }

    QString absoluteFileName = absolutePath(entryPath(m_fileName), fileName);
    const KArchiveEntry* imageFile =
        loadEntry(m_file->xpsArchive(), absoluteFileName, Qt::CaseInsensitive);
    if(!imageFile)
    {
        // image not found
        return QImage();
    ...
    /* WORKAROUND:
        XPS standard requires to use 96dpi for images which doesn't have dpi
       specified (in file). When Qt loads such an image, it sets its dpi to
       qt_defaultDpi and doesn't allow to find out that it happend.

        To workaround this I used this procedure: load image, set its dpi to 96,
       load image again. When dpi isn't set in file, dpi set by me stays
       unchanged.
...
Okular::TextPage* XpsPage::textPage()
{
    ...
    while(!xml.atEnd())
    {
        ...
        if(xml.isStartElement())
        {
            if(xml.name() == QStringLiteral("Canvas"))
            {
                ...
                QString att = xml.attributes()
                                  .value(QStringLiteral("RenderTransform"))
                                  .toString();
                if(!att.isEmpty())
                {
            ...
            else if((xml.name() == QStringLiteral("Canvas.RenderTransform")) ||
                    (xml.name() == QStringLiteral("Glyphs.RenderTransform")))
            {
                useMatrix = true;
            }
            else if(xml.name() == QStringLiteral("MatrixTransform"))
            {
                if(useMatrix)
                {
                    matrix = attsToMatrix(xml.attributes()
                                              .value(QStringLiteral("Matrix"))
                                              .toString()) *
                             matrix;
                }
            }
            else if(xml.name() == QStringLiteral("Glyphs"))
            {
                ...
                glyphsAtts = xml.attributes();
                    ...
                    (xml.name() == QStringLiteral("Path.Fill")) ||
                    (xml.name() == QStringLiteral("SolidColorBrush")) ||
                    (xml.name() == QStringLiteral("ImageBrush")) ||
                    (xml.name() == QStringLiteral("ImageBrush.Transform")) ||
                    (xml.name() == QStringLiteral("Path.OpacityMask")) ||
                    (xml.name() == QStringLiteral("Path.Data")) ||
                    (xml.name() == QStringLiteral("PathGeometry")) ||
                    (xml.name() == QStringLiteral("PathFigure")) ||
                    (xml.name() == QStringLiteral("PolyLineSegment")))
            {
                // those are only graphical - no use in text handling
            }
            else if((xml.name() == QStringLiteral("FixedPage")) ||
                    (xml.name() == QStringLiteral("FixedPage.Resources")))
            {
            ...
            {
                qCWarning(OkularXpsDebug)
                    << "Unhandled element in Text Extraction start: "
                    << xml.name().toString();
            }
        }
        else if(xml.isEndElement())
        {
            ...
            {
                QString att =
                    glyphsAtts.value(QStringLiteral("RenderTransform"))
                        .toString();
                if(!att.isEmpty())
                {
                    matrix = parseRscRefMatrix(att) * matrix;
                }
                QString text = unicodeString(
                    glyphsAtts.value(QStringLiteral("UnicodeString"))
                        .toString());

                // Get font (doesn't work well because qt doesn't allow to load
                // font from file)
                const QString absoluteFileName = absolutePath(
                    entryPath(m_fileName),
                    glyphsAtts.value(QStringLiteral("FontUri")).toString());
                QFont font = m_file->getFontByName(
                    absoluteFileName,
                    glyphsAtts.value(QStringLiteral("FontRenderingEmSize"))
                            .toString()
                            .toFloat() *
                        72 / 96);
                QFontMetrics metrics = QFontMetrics(font);
                // Origin
                QPointF origin(glyphsAtts.value(QStringLiteral("OriginX"))
                                   .toString()
                                   .toDouble(),
                               glyphsAtts.value(QStringLiteral("OriginY"))
                                   .toString()
                                   .toDouble());
                ...
                for(int i = 0; i < text.length(); i++)
                {
                    ...
                    Okular::NormalizedRect* rect = new Okular::NormalizedRect(
                        (origin.x() + lastWidth) / m_pageSize.width(),
            ...
            {
                qCWarning(OkularXpsDebug)
                    << "Unhandled element in Text Extraction end: "
                    << xml.name().toString();
            }
        }
    }
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Error parsing XpsPage text: " << xml.errorString();
    }
    return textPage;
}

void XpsDocument::parseDocumentStructure(
    const QString& documentStructureFileName)
{
    qCWarning(OkularXpsDebug)
        << "document structure file name: " << documentStructureFileName;
    m_haveDocumentStructure = false;

    const KZipFileEntry* documentStructureFile =
        static_cast<const KZipFileEntry*>(
            m_file->xpsArchive()->directory()->entry(
                documentStructureFileName));
    ...
    while(!xml.atEnd())
    {
        ...
        if(xml.isStartElement())
        {
            ...
            {
                // just a container for optional outline and story elements -
                // nothing to do here
            }
            ...
            else if(xml.name() == QStringLiteral("OutlineEntry"))
            {
                ...
                QXmlStreamAttributes attributes = xml.attributes();
                int outlineLevel =
                    attributes.value(QStringLiteral("OutlineLevel"))
                        .toString()
                        .toInt();
                QString description =
                    attributes.value(QStringLiteral("Description")).toString();
                QDomElement synopsisElement =
                    m_docStructure->createElement(description);
                synopsisElement.setAttribute(QStringLiteral("OutlineLevel"),
                                             outlineLevel);
                QString target =
                    attributes.value(QStringLiteral("OutlineTarget"))
                        .toString();
                ...
                synopsisElement.setAttribute(QStringLiteral("Viewport"),
                                             viewport.toString());
                if(outlineLevel == 1)
                {
                ...
                {
                    // find the last next highest element (so it this is level
                    // 3, we need to find the most recent level 2 node)
                    QDomNode maybeParentNode = m_docStructure->lastChild();
                    while(!maybeParentNode.isNull())
                    {
                        if(maybeParentNode.toElement()
                               .attribute(QStringLiteral("OutlineLevel"))
                               .toInt() == (outlineLevel - 1))
                        {
            ...
            {
                // we need to handle Story here, but I have no idea what to do
                // with it.
            }
            else if(xml.name() == QStringLiteral("StoryFragment"))
            {
                // we need to handle StoryFragment here, but I have no idea what
                // to do with it.
            }
            else if(xml.name() == QStringLiteral("StoryFragmentReference"))
            {
                // we need to handle StoryFragmentReference here, but I have no
                // idea what to do with it.
            }
            else
            {
                qCWarning(OkularXpsDebug)
                    << "Unhandled entry in DocumentStructure: "
                    << xml.name().toString();
            }
        }
    }
    ...
    m_docStructure(nullptr)
{
    ...
    const KArchiveEntry* documentEntry =
        file->xpsArchive()->directory()->entry(fileName);
    ...
    while(!docXml.atEnd())
    {
        ...
        if(docXml.isStartElement())
        {
            ...
            {
                QString pagePath = docXml.attributes()
                                       .value(QStringLiteral("Source"))
                                       .toString();
                qCWarning(OkularXpsDebug) << "Page Path: " << pagePath;
                XpsPage* page =
                    new XpsPage(file, absolutePath(documentFilePath, pagePath));
                m_pages.append(page);
            }
            ...
            {
                QString targetName = docXml.attributes()
            ...
            {
                qCWarning(OkularXpsDebug)
                    << "Unhandled entry in FixedDocument: "
                    << docXml.name().toString();
            }
        }
    }
    if(docXml.error())
    {
        qCWarning(OkularXpsDebug) << "Could not parse main XPS document file: "
                                  << docXml.errorString();
    }

    // There might be a relationships entry for this document - typically used
    // to tell us where to find the content structure description

    // We should be able to find this using a reference from some other part of
    // the document, but I can't see it.
    const int slashPosition = fileName.lastIndexOf(QLatin1Char('/'));
    const QString documentRelationshipFile =
        absolutePath(documentEntryPath, QStringLiteral("_rels/") +
                                            fileName.mid(slashPosition + 1) +
                                            QStringLiteral(".rels"));

    const KZipFileEntry* relFile = static_cast<const KZipFileEntry*>(
        file->xpsArchive()->directory()->entry(documentRelationshipFile));
    ...
    if(relFile)
    {
        ...
        while(!xml.atEnd())
        {
            ...
            {
                QXmlStreamAttributes attributes = xml.attributes();
                if(attributes.value(QStringLiteral("Type")).toString() ==
                   QLatin1String("http://schemas.microsoft.com/xps/2005/06/"
                                 "documentstructure"))
                {
                ...
                {
                    qCWarning(OkularXpsDebug)
                        << "Unknown document relationships element: "
                        << attributes.value(QStringLiteral("Type")).toString()
                        << " : "
                        << attributes.value(QStringLiteral("Target"))
                               .toString();
                }
            }
        }
        ...
        {
            qCWarning(OkularXpsDebug)
                << "Could not parse XPS page relationships file ( "
                << documentRelationshipFile << " ) - " << xml.errorString();
        }
    }
    else
    {  // the page relationship file didn't exist in the zipfile
        // this isn't fatal
        qCWarning(OkularXpsDebug)
            << "Could not open Document relationship file from "
            << documentRelationshipFile;
    }

    if(!documentStructureFile.isEmpty())
    {
        // qCWarning(OkularXpsDebug) << "Document structure filename: " <<
        // documentStructureFile; make the document path absolute
        documentStructureFile =
            absolutePath(documentEntryPath, documentStructureFile);
        // qCWarning(OkularXpsDebug) << "Document structure absolute path: " <<
        // documentStructureFile;
...
bool XpsFile::loadDocument(const QString& filename)
{
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Successful open of " << m_xpsArchive->fileName();
    }
    else
    {
        qCWarning(OkularXpsDebug)
            << "Could not open XPS archive: " << m_xpsArchive->fileName();
        delete m_xpsArchive;
        return false;
    }

    // The only fixed entry in XPS is /_rels/.rels
    const KArchiveEntry* relEntry =
        m_xpsArchive->directory()->entry(QStringLiteral("_rels/.rels"));
    if(!relEntry)
    {
        // this might occur if we can't read the zip directory, or it doesn't
        // have the relationships entry
        return false;
    }
    ...
    while(!relXml.atEnd())
    {
        ...
        if(relXml.isStartElement())
        {
            if(relXml.name() == QStringLiteral("Relationship"))
            {
                ...
                QString target =
                    attributes.value(QStringLiteral("Target")).toString();
                if(QStringLiteral("http://schemas.openxmlformats.org/package/"
                                  "2006/relationships/metadata/thumbnail") ==
                   type)
                {
                    m_thumbnailFileName = target;
                }
                else if(QStringLiteral("http://schemas.microsoft.com/xps/2005/"
                                       "06/fixedrepresentation") == type)
                {
                    fixedRepresentationFileName = target;
                }
                else if(QStringLiteral(
                            "http://schemas.openxmlformats.org/package/2006/"
                            "relationships/metadata/core-properties") == type)
                {
                    m_corePropertiesFileName = target;
                }
                else if(QStringLiteral(
                            "http://schemas.openxmlformats.org/package/2006/"
                            "relationships/digital-signature/origin") == type)
                {
                ...
                {
                    qCWarning(OkularXpsDebug)
                        << "Unknown relationships element: " << type << " : "
                        << target;
                }
            }
            ...
            {
                qCWarning(OkularXpsDebug)
                    << "unexpected element in _rels/.rels: "
                    << relXml.name().toString();
            }
        }
    }
    if(relXml.error())
    {
        qCWarning(OkularXpsDebug)
            << "Could not parse _rels/.rels: " << relXml.errorString();
        return false;
    }
    ...
    const KArchiveEntry* fixedRepEntry =
        m_xpsArchive->directory()->entry(fixedRepresentationFileName);
    QString fixedRepresentationFilePath = fixedRepresentationFileName;

    QXmlStreamReader fixedRepXml;
    fixedRepXml.addData(
        readFileOrDirectoryParts(fixedRepEntry, &fixedRepresentationFileName));
    ...
    while(!fixedRepXml.atEnd())
    {
        ...
        if(fixedRepXml.isStartElement())
        {
            ...
            {
                const QString source = fixedRepXml.attributes()
                                           .value(QStringLiteral("Source"))
                                           .toString();
                XpsDocument* doc = new XpsDocument(
                    this, absolutePath(fixedRepresentationFilePath, source));
            ...
            else if(fixedRepXml.name() ==
                    QStringLiteral("FixedDocumentSequence"))
            {
                // we don't do anything here - this is just a container for one
                // or more DocumentReference elements
            }
            else
            {
                qCWarning(OkularXpsDebug)
                    << "Unhandled entry in FixedDocumentSequence: "
                    << fixedRepXml.name().toString();
            }
        }
    }
    if(fixedRepXml.error())
    {
        qCWarning(OkularXpsDebug) << "Could not parse FixedRepresentation file:"
                                  << fixedRepXml.errorString();
        return false;
    }
...
Okular::DocumentInfo XpsFile::generateDocumentInfo() const
{
    ...
    {
        const KZipFileEntry* corepropsFile = static_cast<const KZipFileEntry*>(
            m_xpsArchive->directory()->entry(m_corePropertiesFileName));
        ...
        while(!xml.atEnd())
        {
            ...
            if(xml.isStartElement())
            {
                ...
                {
                    docInfo.set(Okular::DocumentInfo::Description,
                                xml.readElementText());
                }
                ...
                {
                    docInfo.set(Okular::DocumentInfo::Category,
                                xml.readElementText());
                }
                else if(xml.name() == QStringLiteral("created"))
                {
                    ...
                    docInfo.set(
                        ...
                        QLocale().toString(createdDate, QLocale::LongFormat));
                }
                else if(xml.name() == QStringLiteral("modified"))
                {
                    QDateTime modifiedDate = QDateTime::fromString(
                        xml.readElementText(),
                        QStringLiteral("yyyy-MM-ddThh:mm:ssZ"));
                    docInfo.set(
                        Okular::DocumentInfo::ModificationDate,
                        QLocale().toString(modifiedDate, QLocale::LongFormat));
                }
                ...
                {
                    docInfo.set(QStringLiteral("revision"),
                                xml.readElementText(), i18n("Revision"));
                }
            }
        }
        if(xml.error())
        {
            qCWarning(OkularXpsDebug)
                << "Could not parse XPS core properties:" << xml.errorString();
        }
    }
    else
    {
...
bool XpsGenerator::loadDocument(const QString& fileName,
                                QVector<Okular::Page*>& pagesVector)
{
    ...
    for(int docNum = 0; docNum < m_xpsFile->numDocuments(); ++docNum)
    {
        ...
        for(int pageNum = 0; pageNum < doc->numPages(); ++pageNum)
        {
            ...
            pagesVector[pagesVectorOffset] =
                new Okular::Page(pagesVectorOffset, pageSize.width(),
                                 pageSize.height(), Okular::Rotation0);
            ++pagesVectorOffset;
        }
    }
...
Okular::DocumentInfo XpsGenerator::generateDocumentInfo(
...
bool XpsGenerator::exportTo(const QString& fileName,
...
QVariant XpsRenderNode::getRequiredChildData(const QString& name) const
{
    ...
    {
        qCWarning(OkularXpsDebug)
            << "Required element " << name << " is missing in " << this->name;
        return QVariant();
    }
