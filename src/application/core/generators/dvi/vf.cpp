...
void TeXFontDefinition::read_VF_index()
{
    ...
    qCDebug(OkularDviDebug)
        << "TeXFontDefinition::read_VF_index: reading VF pixel file "
        << filename;
#endif
    // Read preamble.
    ...
    if(file_checksum && checksum && file_checksum != checksum)
    {
        qCCritical(OkularDviDebug)
            ...
            << "u, vf = " << file_checksum << "u) in font file" << filename;
    }
    ...
    while((cmnd = one(VF_file)) >= FNTDEF1 && cmnd <= FNTDEF4)
    {
        ...
        Q_UNUSED(design);
        quint16 len =
            one(VF_file) + one(VF_file); /* sequence point in the middle */
        char* fontname = new char[len + 1];
        fread(fontname, sizeof(char), len, VF_file);
        fontname[len] = '\0';

#ifdef DEBUG_FONTS
        qCDebug(OkularDviDebug) << "Virtual font defines subfont \"" << fontname
        ...
        //    TeXFontDefinition *newfontp = font_pool->appendx(fontname,
        //    checksum, (quint32)(scaled_size_in_DVI_units*enlargement_factor),
        //    enlargement_factor);
        TeXFontDefinition* newfontp = font_pool->appendx(
            QString::fromLocal8Bit(fontname), checksum,
            (quint32)((double(scale) / (1 << 20)) * scaled_size_in_DVI_units),
            enlargement_factor);
    ...
    for(; cmnd <= LONG_CHAR; cmnd = one(VF_file))
    {
        ...
        { /* long form packet */
            ...
            {
                qCCritical(OkularDviDebug)
                    << "Virtual character" << cc << "in font" << fontname
                    << "ignored.";
