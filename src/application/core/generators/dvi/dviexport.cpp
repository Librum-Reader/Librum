...
void DVIExport::start(const QString& command, const QStringList& args,
                      ...
                      const QString& error_message)
{
    ...
    connect(process_, &KProcess::readyReadStandardOutput, this,
            &DVIExport::output_receiver);
    connect(process_,
...
DVIExportToPDF::DVIExportToPDF(dviRenderer& parent,
    ...
    DVIExport(parent)
{
    ...
    {
        Q_EMIT error(i18n("<qt><p>Okular could not locate the program "
                          "<em>dvipdfm</em> on your computer. This program is "
                          "essential for the export function to work. You can, "
                          "however, convert "
                          "the DVI-file to PDF using the print function of "
                          "Okular, but that will often "
                          "produce documents which print okay, but are of "
                          "inferior quality if viewed in "
                          "Acrobat Reader. It may be wise to upgrade to a more "
                          "recent version of your "
                          "TeX distribution which includes the "
                          "<em>dvipdfm</em> program.</p>"
                          "<p>Hint to the perplexed system administrator: "
                          "Okular uses the PATH environment variable "
    ...
    start(QStringLiteral("dvipdfm"),
          ...
          i18n("<qt>The external program 'dvipdfm', which was used to export "
               "the file, reported an error. "
               "You might wish to look at the <strong>document info "
               "dialog</strong> which you will "
               "find in the File-Menu for a precise error report.</qt>"));
}

DVIExportToPS::DVIExportToPS(dviRenderer& parent, const QString& output_name,
    ...
    orientation_(orientation)
{
    ...
    {
        Q_EMIT error(
            i18n("<qt>This DVI file refers to external graphic files which are "
                 "not in PostScript format, and cannot be handled by the "
                 "<em>dvips</em> program that Okular uses internally to print "
                 "or to export to PostScript. The functionality that "
                 "you require is therefore unavailable in this version of "
                 "Okular.</qt>"),
            -1);
        return;
    }
    ...
    if(QStandardPaths::findExecutable(QStringLiteral("dvips")).isEmpty())
    {
        Q_EMIT error(
            i18n(
                "<qt><p>Okular could not locate the program <em>dvips</em> on "
                "your computer. "
                "That program is essential for the export function to work.</p>"
                "<p>Hint to the perplexed system administrator: Okular uses "
    ...
    if(!options.isEmpty() || dvi.suggestedPageSize != nullptr)
    {
        ...
        for(parent.current_page = 0; parent.current_page < newFile.total_pages;
            parent.current_page++)
        {
            if(parent.current_page < newFile.total_pages)
            {
                parent.command_pointer =
                    newFile.dvi_Data() +
                    parent.dviFile->page_offset[int(parent.current_page)];
                parent.end_pointer =
                    newFile.dvi_Data() +
                    parent.dviFile->page_offset[int(parent.current_page + 1)];
            }
            else
            {
    ...
    start(QStringLiteral("dvips"), args, QFileInfo(dvi.filename).absolutePath(),
          i18n("<qt>The external program 'dvips', which was used to export the "
    ...
    if(printer_ && !output_name_.isEmpty())
    {
        ...
        if(output.exists() && output.isReadable())
        {
            ...
            Okular::FilePrinter::printFile(
                (*printer_), output_name_, orientation_,
                Okular::FilePrinter::ApplicationDeletesFiles,
                Okular::FilePrinter::ApplicationSelectsPages, QString());
        }
    }
