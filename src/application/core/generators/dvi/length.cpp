...
static const unitOfDistance distanceUnitTable[] = {
    ...
    { float(25.4 / 72.0),
      "bp" },  // big points, 1/72 inch as used in Postscript
...
float Length::convertToMM(const QString& distance, bool* ok)
{
    ...
    for(int i = 0; MMperUnit == 0.0 && distanceUnitTable[i].name != nullptr;
        i++)
    {
        unitPos = distance.lastIndexOf(
            QString::fromLocal8Bit(distanceUnitTable[i].name));
        if(unitPos != -1)
        {
    ...
    if(MMperUnit == 0.0)
    {
        qCCritical(OkularDviShellDebug)
            << "distance::convertToMM: no known unit found in the string '"
            << distance << "'.";
        if(ok)
        {
            *ok = false;
