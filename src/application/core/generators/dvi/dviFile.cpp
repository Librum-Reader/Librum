...
void dvifile::process_preamble()
{
    ...
    if(magic_number != 2)
    {
        errorMsg = i18n("The DVI file contains the wrong version of DVI output "
                        "for this program. "
                        "Hint: If you use the typesetting system Omega, you "
    ...
    cmPerDVIunit = (double(numerator) / double(denominator)) *
                   (double(_magnification) / 1000.0) * (1.0 / 1e5);
...
void dvifile::find_postamble()
{
    ...
    {
        errorMsg = i18n("The DVI file is badly corrupted. Okular was not able "
...
void dvifile::read_postamble()
{
    ...
    while(cmnd >= FNTDEF1 && cmnd <= FNTDEF4)
    {
        ...
        quint32 checksum = readUINT32();  // Checksum of the font, as found by
                                          // TeX in the TFM file
        ...
        quint16 len =
            readUINT8() + readUINT8();  // Length of the font name, including
                                        // the directory name
        QByteArray fontname((char*)command_pointer, len);
        command_pointer += len;

#ifdef DEBUG_FONTS
        qCDebug(OkularDviDebug) << "Postamble: define font \"" << fontname
        ...
        double enlargement_factor = (double(scale) * double(_magnification)) /
                                    (double(design) * 1000.0);

        if(font_pool != nullptr)
        {
            TeXFontDefinition* fontp =
                font_pool->appendx(QString::fromLocal8Bit(fontname), checksum,
                                   scale, enlargement_factor);
...
void dvifile::prepare_pages()
{
    ...
    while(j > 0)
    {
        ...
        {
            errorMsg =
                i18n("The page %1 does not start with the BOP command.", j + 1);
            return;
        }
        command_pointer += 10 * 4;
        page_offset[j] = readUINT32();
        if((dvi_Data() + page_offset[j] < dvi_Data()) ||
           (dvi_Data() + page_offset[j] > dvi_Data() + size_of_file))
        {
            break;
        }
    }
...
void dvifile::pdf2psNotFound(const QString& PDFFilename, QString* converrorms)
{
    ...
    {
        *converrorms = i18n(
            "<qt><p>The external program <strong>pdf2ps</strong> could not be "
            "started. As a result, "
            "the PDF-file %1 could not be converted to PostScript. Some "
            "graphic elements in your "
            "document will therefore not be displayed.</p>"
            "<p><b>Possible reason:</b> The program <strong>pdf2ps</strong> "
            "may not be installed "
            "on your system, or cannot be found in the current search path.</p>"
            "<p><b>What you can do:</b> The program <strong>pdf2ps</strong> is "
            "normally "
            "contained in distributions of the ghostscript PostScript "
            "interpreter system. If "
            "ghostscript is not installed on your system, you could install it "
            "now. "
            "If you are sure that ghostscript is installed, try to use "
            "<strong>pdf2ps</strong> "
            "from the command line to check if it really "
            "works.</p><p><em>PATH:</em> %2</p></qt>",
            PDFFilename, QString::fromLocal8Bit(qgetenv("PATH")));
                                ...
                                QString* converrorms)
{
    ...
    static const QString fullPath =
        QStandardPaths::findExecutable(QStringLiteral("pdf2ps"));
    if(!fullPath.isEmpty())
    {
    ...
    pdf2ps.start(fullPath, QStringList() << PDFFilename << convertedFileName,
    ...
    if(!QFile::exists(convertedFileName) || pdf2ps.exitCode() != 0)
    {
        ...
        if(converrorms != nullptr)
        {
            ...
            *converrorms =
                i18n("<qt><p>The PDF-file %1 could not be converted to "
                     "PostScript. Some graphic elements in your "
                     "document will therefore not be displayed.</p>"
                     "<p><b>Possible reason:</b> The file %1 might be broken, "
                     "or might not be a PDF-file at all. "
                     "This is the output of the <strong>pdf2ps</strong> "
                     "program that Okular used:</p>"
