    ...
    TeXFont(parent)
{
    ...
    qCDebug(OkularDviDebug)
        << "TeXFont_TFM::TeXFont_TFM( parent=" << parent << " )";
#endif
    ...
    if(!file.open(QIODevice::ReadOnly))
    {
        qCCritical(OkularDviDebug)
            << "TeXFont_TFM::TeXFont_TFM(): Could not read TFM file";
        return;
    }
    ...
    {
        qCCritical(OkularDviDebug)
            << "TeXFont_TFM::TeXFont_TFM( filename=" << parent->filename
            << " ): The font has an invalid bc and ec entries.";
        file.close();
        return;
    }
    ...
    qCDebug(OkularDviDebug)
        << "checksum    = " << checksum
        << "design_size = " << design_size_in_TeX_points.toDouble()
        << " TeX Points\n"
        << "            = "
    ...
    // Width table
    fix_word widthTable_in_units_of_design_size
    ...
    for(unsigned int characterCode = bc; characterCode < ec; characterCode++)
    {
        ...
        if(byte >= nw)
        {
            qCCritical(OkularDviDebug)
                << "TeXFont_TFM::TeXFont_TFM( filename=" << parent->filename
                << " ): The font has an invalid Char-Info table.";
        }
        else
        {
            characterWidth_in_units_of_design_size[characterCode] =
                widthTable_in_units_of_design_size[byte];
            g->dvi_advance_in_units_of_design_size_by_2e20 =
                widthTable_in_units_of_design_size[byte].value;
        }
        ...
        {
            characterHeight_in_units_of_design_size[characterCode] =
                heightTable_in_units_of_design_size[byte];
        }
...
glyph* TeXFont_TFM::getGlyph(quint16 characterCode,
                             bool generateCharacterPixmap, const QColor& color)
{
#ifdef DEBUG_TFM
    qCDebug(OkularDviDebug)
        << "TeXFont_TFM::getGlyph( ch=" << ch
    ...
    {
        qCCritical(OkularDviDebug)
            << "TeXFont_TFM::getGlyph(): Argument is too big.";
        return glyphtable;
    }
       ...
       ((g->shrunkenCharacter.isNull()) || (color != g->color)))
    {
        g->color = color;
        quint16 pixelWidth =
            (quint16)(parent->displayResolution_in_dpi *
        ...
        g->shrunkenCharacter =
            QImage(pixelWidth, pixelHeight, QImage::Format_RGB32);
