    ...
    TeXFont(parent)
{
    ...
    if(file == nullptr)
    {
        qCCritical(OkularDviDebug)
            << i18n("Cannot open font file %1.", parent->filename);
    }
#ifdef DEBUG_PK
    else
        qCDebug(OkularDviDebug)
                            ...
                            const QColor& color)
{
    ...
    {
        qCCritical(OkularDviDebug)
            << "TeXFont_PK::getGlyph(): Argument is too big.";
        return glyphtable;
    }
       ...
       (characterBitmaps[ch]->w != 0))
    {
        ...
        double srcXTrans =
            shrinkFactor * (g->x / shrinkFactor - ceil(g->x / shrinkFactor));
        ...
        // in other words,
        int shrunk_width =
            (int)ceil((characterBitmaps[ch]->w - srcXTrans) / shrinkFactor);

        // Now do the same for the y-coordinate
        g->y2 = (int)ceil(g->y / shrinkFactor);
        double srcYTrans =
            shrinkFactor * (g->y / shrinkFactor - ceil(g->y / shrinkFactor));
        int shrunk_height =
            (int)ceil((characterBitmaps[ch]->h - srcYTrans) / shrinkFactor);
        ...
        for(int x = 0; x < characterBitmaps[ch]->w; x++)
        {
            ...
            {
                quint8 bit = *(characterBitmaps[ch]->bits +
                               characterBitmaps[ch]->bytes_wide * y + (x >> 3));
                bit = bit >> (x & 7);
                bit = bit & 1;
        ...
        for(int y = 0; y < characterBitmaps[ch]->h; y++)
        {
            for(int x = 0; x < shrunk_width; x++)
            {
                ...
                for(int srcX = (int)ceil(destStartX); srcX < floor(destEndX);
                    srcX++)
                {
                ...
                if(destStartX >= 0.0)
                {
                    value += (quint32)(255.0 * (ceil(destStartX) - destStartX) *
                                       data[characterBitmaps[ch]->w * y +
                                            (int)floor(destStartX)]);
                }
                if(floor(destEndX) < characterBitmaps[ch]->w)
                {
                    value += (quint32)(255.0 * (destEndX - floor(destEndX)) *
                                       data[characterBitmaps[ch]->w * y +
                                            (int)floor(destEndX)]);
                }
        ...
        for(int x = 0; x < shrunk_width; x++)
        {
            for(int y = 0; y < shrunk_height; y++)
            {
                ...
                for(int srcY = (int)ceil(destStartY); srcY < floor(destEndY);
                    srcY++)
                {
                ...
                {
                    value +=
                        (quint32)((ceil(destStartY) - destStartY) *
                                  xdata[shrunk_width * (int)floor(destStartY) +
                                        x]);
                }
                if(floor(destEndY) < characterBitmaps[ch]->h)
                {
                    value +=
                        (quint32)((destEndY - floor(destEndY)) *
                                  xdata[shrunk_width * (int)floor(destEndY) +
                                        x]);
                }
        ...
        else
        {
            ...
            for(int y = 0; y < shrunk_height; y++)
            {
                unsigned int* destScanLine =
                    reinterpret_cast<unsigned int*>(im32.scanLine(y));
                for(int col = 0; col < shrunk_width; col++)
                {
    ...
    do
    {
        ...
        if(PK_flag_byte >= PK_CMD_START)
        {
            ...
            case PK_NOOP:
                break;
            default:
                oops(i18n("Unexpected %1 in PK file %2", PK_flag_byte,
                          parent->filename));
                break;
            }
        }
...
void TeXFont_PK::read_PK_char(unsigned int ch)
{
    ...
    (void)num(fp, n); /* vertical escapement */
    {
        ...
        {
            oops(i18n("The character %1 is too large in file %2", ch,
                      parent->filename));
        }
...

    {
        ...
        characterBitmaps[ch]->bytes_wide =
            ROUNDUP((int)characterBitmaps[ch]->w, 32) * 4;
        unsigned int size =
            characterBitmaps[ch]->bytes_wide * characterBitmaps[ch]->h;
    ...
    if(QSysInfo::ByteOrder == QSysInfo::BigEndian)
    {
        ...
        if(PK_dyn_f == 14)
        { /* get raster by bits */
            memset(characterBitmaps[ch]->bits, 0,
                   (int)characterBitmaps[ch]->h * bytes_wide);
            for(i = 0; i < (int)characterBitmaps[ch]->h; i++)
            { /* get all rows */
                cp = ADD(characterBitmaps[ch]->bits, i * bytes_wide);
                row_bit_pos = 32;
                for(j = 0; j < (int)characterBitmaps[ch]->w; j++)
                { /* get one row */
                    if(--PK_bitpos < 0)
                    {
                        word = one(fp);
        ...
        { /* get packed raster */
            ...
            if(cp != (reinterpret_cast<quint32*>(characterBitmaps[ch]->bits +
                                                 bytes_wide *
                                                     characterBitmaps[ch]->h)))
            {
                oops(i18n("Wrong number of bits stored:  char. %1, font %2", ch,
                          parent->filename));
            }
...
void TeXFont_PK::read_PK_index()
{
    ...
    {
        qCCritical(OkularDviDebug)
            << "TeXFont_PK::read_PK_index(): file is not a PK file";
        return;
    }
    ...
    if(hppp != vppp)
    {
        qCWarning(OkularDviDebug)
            << i18n("Font has non-square aspect ratio ") << vppp << ":" << hppp;
    }
...
#ifdef DEBUG_PK
        qCDebug(OkularDviDebug)
            << "Scanning pk char " << ch << "at " << glyphtable[ch].addr;
#endif
    }
#ifdef DEBUG_PK
    qCDebug(OkularDviDebug) << "TeXFont_PK::read_PK_index() called";
