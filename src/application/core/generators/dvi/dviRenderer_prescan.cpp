...
extern void parse_special_argument(const QString& strg,
                                   const char* argument_name, int* variable);
...
void dviRenderer::prescan_embedPS(char* cp, quint8* beginningOfSpecialCommand)
{
    ...
    if((EPSfilename.at(0) == QLatin1Char('"')) &&
       (EPSfilename.at(EPSfilename.length() - 1) == QLatin1Char('"')))
    {
    ...
    bool const is_ps_file =
        (mime_type_name == QLatin1String("application/postscript") ||
         mime_type_name == QLatin1String("image/x-eps"));
    bool const is_pdf_file =
        (!is_ps_file && mime_type_name == QLatin1String("application/pdf"));
    ...
    if(!QFile::exists(EPSfilename))
    {
        ...
        {
            if((dviFile->page_offset[page] <= currentOffset) &&
               (currentOffset <= dviFile->page_offset[page + 1]))
            {
                break;
            }
        }
        if(is_pdf_file)
        {
            errorMsg += i18n("Page %1: The PDF file <strong>%2</strong> could "
                             "not be converted to PostScript.<br>",
                             page + 1, originalFName);
        }
        else
        {
            errorMsg += i18n("Page %1: The PostScript file <strong>%2</strong> "
    ...
    int clip = include_command.indexOf(QStringLiteral(
        " clip"));  // -1 if clip keyword is not present, >= 0 otherwise

    // Generate the PostScript commands to be included
    QString PS =
        QStringLiteral("ps: @beginspecial %1 @llx %2 @lly %3 @urx %4 @ury")
            .arg(llx)
            .arg(lly)
            .arg(urx)
            .arg(ury);
    ...
    QVector<quint8> newDVI(dviFile->size_of_file + lengthOfNewSpecial -
                           lengthOfOldSpecial);

    quint8* commandPtrSav = command_pointer;
    quint8* endPtrSav = end_pointer;
    end_pointer = newDVI.data() + dviFile->size_of_file + lengthOfNewSpecial -
                  lengthOfOldSpecial;
    memcpy(newDVI.data(), dviFile->dvi_Data(),
           beginningOfSpecialCommand - dviFile->dvi_Data());
    command_pointer =
        newDVI.data() + (beginningOfSpecialCommand - dviFile->dvi_Data());
    command_pointer[0] = XXX4;
    command_pointer++;
    writeUINT32(PS.length());
    memcpy(
        newDVI.data() + (beginningOfSpecialCommand - dviFile->dvi_Data()) + 5,
        PS.toLatin1().constData(), PS.length());
    memcpy(newDVI.data() + (beginningOfSpecialCommand - dviFile->dvi_Data()) +
               lengthOfNewSpecial,
           beginningOfSpecialCommand + lengthOfOldSpecial,
           dviFile->size_of_file -
               (beginningOfSpecialCommand - dviFile->dvi_Data()) -
               lengthOfOldSpecial);

    // Adjust page pointers in the DVI file
    dviFile->size_of_file =
        dviFile->size_of_file + lengthOfNewSpecial - lengthOfOldSpecial;
    ...
    for(int i = 0; i < dviFile->total_pages; i++)
    {
        ...
        {
            dviFile->page_offset[i] = dviFile->page_offset[i] +
                                      lengthOfNewSpecial - lengthOfOldSpecial;
            command_pointer =
                dviFile->page_offset[i] + newDVI.data() + 4 * 10 + 1;
    ...
    dviFile->beginning_of_postamble = dviFile->beginning_of_postamble +
                                      lengthOfNewSpecial - lengthOfOldSpecial;
    dviFile->page_offset[int(dviFile->total_pages)] =
        dviFile->beginning_of_postamble;
    ...
    command_pointer = newDVI.data() + (command_pointer - dviFile->dvi_Data()) +
                      lengthOfNewSpecial - lengthOfOldSpecial;
    end_pointer = newDVI.data() + (end_pointer - dviFile->dvi_Data()) +
                  lengthOfNewSpecial - lengthOfOldSpecial;
...
void dviRenderer::prescan_removePageSizeInfo(char* cp,
                                             quint8* beginningOfSpecialCommand)
{
#ifdef DEBUG_PRESCAN
    qCDebug(OkularDviDebug)
        << "dviRenderer::prescan_embedPS( cp = " << cp << " ) ";
...
void dviRenderer::prescan_ParsePapersizeSpecial(const QString& _cp)
{
    ...
    else
    {
        printErrorMsgForSpecials(
            i18n("The papersize data '%1' could not be parsed.", cp));
    }
...
void dviRenderer::prescan_ParsePSHeaderSpecial(const QString& cp)
{
    ...
    if(QFile::exists(_file))
    {
        PS_interface->PostScriptHeaderString->append(
            QStringLiteral(" (%1) run\n").arg(_file));
    }
}
...
void dviRenderer::prescan_ParsePSBangSpecial(const QString& cp)
{
    ...
    PS_interface->PostScriptHeaderString->append(
        QStringLiteral(" @defspecial \n"));
    PS_interface->PostScriptHeaderString->append(cp);
    PS_interface->PostScriptHeaderString->append(
        QStringLiteral(" @fedspecial \n"));
}
...
void dviRenderer::prescan_ParsePSQuoteSpecial(const QString& cp)
{
    ...
    PostScriptOutPutString->append(
        QStringLiteral(" %1 %2 moveto\n").arg(PS_H).arg(PS_V));
    PostScriptOutPutString->append(
        QStringLiteral(" @beginspecial @setspecial \n"));
...
void dviRenderer::prescan_ParsePSSpecial(const QString& cp)
{
    ...
    if(cp.startsWith(QLatin1String("ps:SDict begin")))
    {
        ...
        if(cp.startsWith(QLatin1String(
               "ps:SDict begin /product where{pop product(Distiller)")))
        {
            return;  // hyperref tries to work around Distiller bug
        }
        if(cp.startsWith(QLatin1String("ps:SDict begin [")) &&
           cp.endsWith(QLatin1String(" pdfmark end")))
        {  // hyperref definition of link/anchor/bookmark/etc
            if(cp.contains(QStringLiteral("/DEST")))
            {  // The PostScript code defines an anchor
                QString anchorName = cp.section(QLatin1Char('('), 1, 1)
                                         .section(QLatin1Char(')'), 0, 0);
                Length l;
                l.setLength_in_inch(currinf.data.dvi_v /
                                    (resolutionInDPI * shrinkfactor));
                anchorList[anchorName] = Anchor(current_page + 1, l);
            }
            // The PostScript code defines a bookmark
            if(cp.contains(QStringLiteral("/Dest")) &&
               cp.contains(QStringLiteral("/Title")))
            {
                const QString childrenNumberAndMoreStuff =
                    cp.section(QLatin1Char('-'), 1,
                               1);  // Contains from the - symbol to the end of
                                    // cp, effectively containing the number of
                                    // children and some stuff after it
                ...
                prebookmarks.append(PreBookmark(
                    PDFencodingToQString(cp.section(QLatin1Char('('), 2, 2)
                                             .section(QLatin1Char(')'), 0, 0)),
                    cp.section(QLatin1Char('('), 1, 1)
                        .section(QLatin1Char(')'), 0, 0),
                    childrenNumberAndMoreStuff.leftRef(indexOfFirstNonDigit)
                        .toUInt()));
            }
            return;
        }
...
void dviRenderer::prescan_ParsePSFileSpecial(const QString& cp)
{
    ...
    if((ending == QLatin1String("png")) || (ending == QLatin1String("gif")) ||
       (ending == QLatin1String("jpg")) || (ending == QLatin1String("jpeg")))
    {
    ...
    if(QFile::exists(EPSfilename))
    {
        ...
        {
            PostScriptOutPutString->append(
                QStringLiteral(" %1 @angle").arg(angle));
        }
        if(clip != -1)
        {
...
void dviRenderer::prescan_ParseSourceSpecial(const QString& cp)
{
    ...
    QString sourceFileName =
        QFileInfo(fi1.dir(), cp.mid(j).trimmed()).absoluteFilePath();
    Length l;
    l.setLength_in_inch(currinf.data.dvi_v / (resolutionInDPI * shrinkfactor));
    DVI_SourceFileAnchor sfa(sourceFileName, sourceLineNumber, current_page + 1,
                             l);
...
void dviRenderer::prescan_setChar(unsigned int ch)
{
    ...
    {
        glyph* g =
            ((TeXFont*)(currinf.fontp->font))->getGlyph(ch, true, globalColor);
        if(g == nullptr)
        {
            return;
        }
        currinf.data.dvi_h +=
            (int)(currinf.fontp->scaled_size_in_DVI_units *
                      dviFile->getCmPerDVIunit() * (1200.0 / 2.54) / 16.0 *
    ...
    if(currinf.set_char_p == &dviRenderer::set_vf_char)
    {
        ...
        currinf.data.dvi_h +=
            (int)(currinf.fontp->scaled_size_in_DVI_units *
                      dviFile->getCmPerDVIunit() * (1200.0 / 2.54) / 16.0 *
                      m->dvi_advance_in_units_of_design_size_by_2e20 +
                  0.5);
        return;
    }
}
...
void dviRenderer::prescan(parseSpecials specialParser)
{
    ...
    for(;;)
    {
        ...
        if(FNTNUM0 <= ch && ch <= (unsigned char)(FNTNUM0 + 63))
        {
            ...
            {
                errorMsg = i18n("The DVI code referred to font #%1, which was "
        ...
        case BOP:
            ...
            currinf.data.dvi_h = 1200 << 16;  // Reminder: DVI-coordinates start
                                              // at (1",1") from top of page
            currinf.data.dvi_v = 1200;
            currinf.data.pxl_v = int(currinf.data.dvi_v / shrinkfactor);
            currinf.data.w = currinf.data.x = currinf.data.y = currinf.data.z =
                0;
            break;
        ...
        case RIGHT4:
            ...
            currinf.data.dvi_h +=
                ((long)(RRtmp * 65536.0 * fontPixelPerDVIunit));
            break;
        ...
        case DOWN4:
        {
            ...
            currinf.data.dvi_v +=
                ((long)(DDtmp * 65536.0 * fontPixelPerDVIunit)) / 65536;
            currinf.data.pxl_v = int(currinf.data.dvi_v / shrinkfactor);
        }
        break;
