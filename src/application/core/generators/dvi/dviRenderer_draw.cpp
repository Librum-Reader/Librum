...
void dviRenderer::set_char(unsigned int cmd, unsigned int ch)
{
    ...
    {
        g = ((TeXFont*)(currinf.fontp->font))
                ->getGlyph(ch, true, colorStack.top());
    }
    if(g == nullptr)
    {
        return;
    ...
    if(HTML_href != nullptr)
    {
        ...
        else
        {
            QRect dshunion =
                currentlyDrawnPage
                    ->hyperLinkList[currentlyDrawnPage->hyperLinkList.size() -
                                    1]
                    .box.united(QRect(x, y, pix.width(), pix.height()));
            currentlyDrawnPage
                ->hyperLinkList[currentlyDrawnPage->hyperLinkList.size() - 1]
                .box = dshunion;
        }
    }
    ...
    RenderedDviPagePixmap* currentDVIPage =
        dynamic_cast<RenderedDviPagePixmap*>(currentlyDrawnPage);
    if(source_href != nullptr && currentDVIPage)
    {
        ...
        else
        {
            QRect dshunion =
                currentDVIPage
                    ->sourceHyperLinkList
                        [currentDVIPage->sourceHyperLinkList.size() - 1]
                    .box.united(QRect(x, y, pix.width(), pix.height()));
            currentDVIPage
                ->sourceHyperLinkList
                    [currentDVIPage->sourceHyperLinkList.size() - 1]
    ...
    case 0x0b:
        currentlyDrawnPage
            ->textBoxList[currentlyDrawnPage->textBoxList.size() - 1]
            .text += QLatin1String("ff");
        break;
    case 0x0c:
        currentlyDrawnPage
            ->textBoxList[currentlyDrawnPage->textBoxList.size() - 1]
            .text += QLatin1String("fi");
        break;
    case 0x0d:
        currentlyDrawnPage
            ->textBoxList[currentlyDrawnPage->textBoxList.size() - 1]
            .text += QLatin1String("fl");
        break;
    case 0x0e:
        currentlyDrawnPage
            ->textBoxList[currentlyDrawnPage->textBoxList.size() - 1]
            .text += QLatin1String("ffi");
        break;
    case 0x0f:
        currentlyDrawnPage
            ->textBoxList[currentlyDrawnPage->textBoxList.size() - 1]
            .text += QLatin1String("ffl");
        break;
...
void dviRenderer::set_vf_char(unsigned int cmd, unsigned int ch)
{
    ...
    qCDebug(OkularDviDebug)
        << "dviRenderer::set_vf_char( cmd=" << cmd << ", ch=" << ch << " )";
#endif
    ...
    if(m->pos == nullptr)
    {
        qCCritical(OkularDviDebug)
            << "Character " << ch << " not defined in font "
    ...
    end_pointer = m->end;
    draw_part(currinf.fontp->scaled_size_in_DVI_units *
                  (dviFile->getCmPerDVIunit() * 1200.0 / 2.54) / 16.0,
              true);
...
#ifdef DEBUG_RENDER
    qCDebug(OkularDviDebug)
        << "dviRenderer::set_no_char( cmd=" << cmd << ", ch =" << ch << " )";
#endif
    ...
    for(;;)
    {
        ...
        else
        {
            ...
            case SETRULE:
                ...
                if(a > 0 && b > 0)
                {
                    int h = ((int)ROUNDUP(((long)(a * current_dimconv)),
                                          shrinkfactor * 65536));
                    int w = ((int)ROUNDUP(b, shrinkfactor * 65536));

                    if(colorStack.isEmpty())
                    {
                        foreGroundPainter->fillRect(
                            ((int)((currinf.data.dvi_h) /
                                   (shrinkfactor * 65536))),
                            currinf.data.pxl_v - h + 1, w ? w : 1, h ? h : 1,
                            globalColor);
                    }
                    else
                    {
            ...
            case BOP:
                ...
                currinf.data.dvi_h = 1200
                                     << 16;  // Reminder: DVI-coordinates start
                                             // at (1",1") from top of page
                currinf.data.dvi_v = 1200;
                currinf.data.pxl_v = int(currinf.data.dvi_v / shrinkfactor);
                currinf.data.w = currinf.data.x = currinf.data.y =
                    currinf.data.z = 0;
                break;
            ...
            case EOP:
                ...
                if(is_vfmacro == false)
                {
                    ...
                    {
                        qCDebug(OkularDviDebug)
                            << "DRAW: The stack was not empty when the EOP "
                               "command was encountered.";
                        errorMsg = i18n("The stack was not empty when the EOP "
                                        "command was encountered.");
                        return;
                    }
                }
            ...
            case POP:
                if(stack.isEmpty())
                {
                    errorMsg = i18n("The stack was empty when a POP command "
                                    "was encountered.");
                    return;
                }
                else
            ...
            case RIGHT4:
                ...
                if((is_vfmacro == false) && (currinf.fontp != nullptr) &&
                   ((RRtmp >= currinf.fontp->scaled_size_in_DVI_units / 6) ||
                    (RRtmp <=
                     -4 * (currinf.fontp->scaled_size_in_DVI_units / 6))) &&
            ...
            case W0:
                if((is_vfmacro == false) && (currinf.fontp != nullptr) &&
                   ((WWtmp >= currinf.fontp->scaled_size_in_DVI_units / 6) ||
                    (WWtmp <=
                     -4 * (currinf.fontp->scaled_size_in_DVI_units / 6))) &&
                   (currentlyDrawnPage->textBoxList.size() > 0))
                {
            ...
            case X0:
                if((is_vfmacro == false) && (currinf.fontp != nullptr) &&
                   ((XXtmp >= currinf.fontp->scaled_size_in_DVI_units / 6) ||
                    (XXtmp <=
                     -4 * (currinf.fontp->scaled_size_in_DVI_units / 6))) &&
                   (currentlyDrawnPage->textBoxList.size() > 0))
                {
            ...
            case DOWN4:
            {
                ...
                if((is_vfmacro == false) && (currinf.fontp != nullptr) &&
                   (abs(DDtmp) >=
                    5 * (currinf.fontp->scaled_size_in_DVI_units / 6)) &&
                   (currentlyDrawnPage->textBoxList.size() > 0))
                {
                    word_boundary_encountered = true;
                    line_boundary_encountered = true;
                    space_encountered = true;
                    if(abs(DDtmp) >=
                       10 * (currinf.fontp->scaled_size_in_DVI_units / 6))
                    {
                        currentlyDrawnPage
                            ->textBoxList
                                [currentlyDrawnPage->textBoxList.size() - 1]
                            .text += QLatin1Char('\n');
                    }
                }
            ...
            case FNT3:
                currinf.fontp =
                    currinf.fonttable->value(readUINT(ch - FNT1 + 1));
                if(currinf.fontp == nullptr)
                {
                    errorMsg = i18n("The DVI code referred to a font which was "
                                    "not previously defined.");
                    return;
                }
                currinf.set_char_p = currinf.fontp->set_char_p;
            ...
            case FNT4:
                currinf.fontp =
                    currinf.fonttable->value(readINT(ch - FNT1 + 1));
                if(currinf.fontp == nullptr)
                {
                    errorMsg = i18n("The DVI code referred to a font which was "
                                    "not previously defined.");
        ...
        if(currentlyDrawnPage->textBoxList.size() > 0)
            qCDebug(OkularDviDebug)
                << "Element:" << currentlyDrawnPage->textBoxList.last().box
        ...
        if(after_space && line_boundary_encountered &&
           word_boundary_encountered)
        {
        ...
        {
            for(int lidx = last_space_index + 1;
                lidx < currentlyDrawnPage->textBoxList.size(); ++lidx)
            {
                // merge two adjacent boxes which are part of the same word
                currentlyDrawnPage->textBoxList[lidx - 1].box.setRight(
                    currentlyDrawnPage->textBoxList[lidx].box.x());
            }
#ifdef DEBUG_RENDER
            QString lastword(
                currentlyDrawnPage->textBoxList[last_space_index].text);
            for(int lidx = last_space_index + 1;
                lidx < currentlyDrawnPage->textBoxList.size(); ++lidx)
                lastword += currentlyDrawnPage->textBoxList[lidx].text;
            qCDebug(OkularDviDebug)
                << "space encountered: '" << lastword << "'";
#endif
...
void dviRenderer::draw_page()
{
    ...
    {
        qCDebug(OkularDviDebug) << "Time elapsed till the first page is drawn: "
                                << performanceTimer.restart() << "ms";
        performanceFlag = 1;
    }
#endif
    ...
    foreGroundPainter->fillRect(foreGroundPainter->viewport(),
    ...
    if(_postscript)
    {
        ...
        PS_interface->graphics(current_page, resolutionInDPI,
                               dviFile->getMagnification(), foreGroundPainter);
    }
    ...
    if(current_page < dviFile->total_pages)
    {
        ...
        end_pointer =
            dviFile->dvi_Data() + dviFile->page_offset[int(current_page + 1)];
    }
    else
    {
