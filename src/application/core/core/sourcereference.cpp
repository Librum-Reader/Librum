...
bool Okular::extractLilyPondSourceReference(const QUrl& url, QString* file,
                                            int* row, int* col)
{
    ...
    int columnEnd =
        parts.takeLast().toInt(&ok);  // apparently we don't use this
    Q_UNUSED(columnEnd);
    if(!ok)
    {
        return false;
    ...
    *row = parts.takeLast().toInt(&ok);
    ...
    // In case the path itself contains :, we need to reconstruct it after
