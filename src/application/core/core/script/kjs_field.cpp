...
static QString fieldGetTypeHelper(const FormField* field)
{
    ...
    {
        const FormFieldButton* button =
            static_cast<const FormFieldButton*>(field);
    ...
    {
        const FormFieldChoice* choice =
            static_cast<const FormFieldChoice*>(field);
...
static KJSObject fieldGetValue(KJSContext* /*context*/, void* object)
{
    ...
    case FormField::FormChoice:
    {
        ...
        {
            return KJSString(choice->exportValueForChoice(
                choice->choices().at(currentChoices[0])));
        }
        break;
    }
    case FormField::FormSignature:
...
static KJSObject fieldGetDisplay(KJSContext*, void* object)
{
    ...
    if(visible)
    {
        return KJSNumber(field->isPrintable() ? FormDisplay::FormVisible
                                              : FormDisplay::FormNoPrint);
    }
    return KJSNumber(field->isPrintable() ? FormDisplay::FormNoView
                                          : FormDisplay::FormHidden);
}
                                    ...
                                    const KJSArguments& arguments)
{
    ...
    const QString fieldName =
        arguments.at(0).property(ctx, OKULAR_NAME).toString(ctx);
...
void JSField::initType(KJSContext* ctx)
{
    ...
    g_fieldProto->defineProperty(ctx, QStringLiteral("value"), fieldGetValue,
                                 fieldSetValue);
    g_fieldProto->defineProperty(ctx, QStringLiteral("hidden"), fieldGetHidden,
                                 fieldSetHidden);
    g_fieldProto->defineProperty(ctx, QStringLiteral("display"),
                                 fieldGetDisplay, fieldSetDisplay);

    g_fieldProto->defineFunction(ctx, QStringLiteral("buttonGetIcon"),
                                 fieldButtonGetIcon);
    g_fieldProto->defineFunction(ctx, QStringLiteral("buttonSetIcon"),
                                 fieldButtonSetIcon);
}
