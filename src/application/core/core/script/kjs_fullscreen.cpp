...
void JSFullscreen::initType(KJSContext* ctx)
{
    ...
    g_fsProto->defineProperty(ctx, QStringLiteral("loop"), fsGetLoop,
                              fsSetLoop);
    g_fsProto->defineProperty(ctx, QStringLiteral("useTimer"), fsGetUseTimer,
                              fsSetUseTimer);
    g_fsProto->defineProperty(ctx, QStringLiteral("timeDelay"), fsGetTimeDelay,
                              fsSetTimeDelay);
}
