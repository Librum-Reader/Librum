...
public:
    ...
    {
        SignatureStatusUnknown,  ///< The signature status is unknown for some
                                 ///< reason.
        SignatureValid,  ///< The signature is cryptographically valid.
        SignatureInvalid,  ///< The signature is cryptographically invalid.
        SignatureDigestMismatch,  ///< The document content was changed after
        ...
        SignatureNotFound,  ///< The requested signature is not present in the
                            ///< document.
    ...
    {
        CertificateStatusUnknown,  ///< The certificate status is unknown for
                                   ///< some reason.
        ...
        CertificateUnknownIssuer,  ///< The certificate trust chain has not
                                   ///< finished in a trusted root certificate.
        CertificateRevoked,  ///< The certificate was revoked by the issuing
                             ///< certificate authority.
        CertificateExpired,  ///< The signing time is outside the validity
                             ///< bounds of this certificate.
...
public:
     ...
     *
     * This can ask the user for a password, userCancelled will be true if the
     * user decided not to enter it.
     */
    virtual QList<CertificateInfo*> signingCertificates(
        bool* userCancelled) const;

    /**
     * Returns list of valid, usable signing certificates for current date and
     * time.
     *
     * This can ask the user for a password, userCancelled will be true if the
     * user decided not to enter it.
     *
     * nonDateValidCerts is true if the user has signing certificates but their
     * validity start date is in the future or past their validity end date.
     */
    QList<CertificateInfo*> signingCertificatesForNow(
