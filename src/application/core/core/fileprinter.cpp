...
Document::PrintError FilePrinter::printFile(
    ...
    const QString& pageRange, ScaleMode scaleMode)
{
    ...
    return fp.doPrintFiles(printer, QStringList(file), fileDeletePolicy,
                           pageSelectPolicy, pageRange, documentOrientation,
                           scaleMode);
}

static Document::PrintError doKProcessExecute(const QString& exe,
...
Document::PrintError FilePrinter::doPrintFiles(
    QPrinter& printer, const QStringList& fileList,
    FileDeletePolicy fileDeletePolicy, PageSelectPolicy pageSelectPolicy,
    const QString& pageRange, QPrinter::Orientation documentOrientation,
    ScaleMode scaleMode)
{
    if(fileList.size() < 1)
    {
    ...
    if(!printer.outputFileName().isEmpty())
    {
        ...
        else if(inputFileInfo.suffix() == QLatin1String("ps") &&
                ...
                ps2pdfAvailable())
        {
            ...
            qCDebug(OkularCoreDebug)
                << "Executing" << exe << "with arguments" << argList;
            ret = doKProcessExecute(exe, argList);
        }
        else if(inputFileInfo.suffix() == QLatin1String("pdf") &&
    ...
    {  // Print to a printer via lpr command
        ...
        // Decide what executable to use to print with, need the CUPS version of
        // lpr if available Some distros name the CUPS version of lpr as
        // lpr-cups or lpr.cups so try those first before default to lpr, or
        // failing that to lp

        if(!QStandardPaths::findExecutable(QStringLiteral("lpr-cups"))
                .isEmpty())
        {
            exe = QStringLiteral("lpr-cups");
        }
        else if(!QStandardPaths::findExecutable(QStringLiteral("lpr.cups"))
                     .isEmpty())
        {
            exe = QStringLiteral("lpr.cups");
        }
        else if(!QStandardPaths::findExecutable(QStringLiteral("lpr"))
                     .isEmpty())
        {
...
QString FilePrinter::pageRange(QPrinter& printer, int lastPage,
                               const QList<int>& selectedPageList)
{
    if(printer.printRange() == QPrinter::Selection)
    {
...
QString FilePrinter::pageListToPageRange(const QList<int>& pageList)
{
    ...
    while(i != count)
    {
        if(i + 1 == count || pageList[i] + 1 != pageList[i + 1])
        {
            ...
            {
                pageRange.append(
                    QStringLiteral("%1-%2").arg(seqStart).arg(seqEnd));
            }
...
{
    return (
        !QStandardPaths::findExecutable(QStringLiteral("ps2pdf")).isEmpty());
}

bool FilePrinter::pdf2psAvailable()
{
    return (
        !QStandardPaths::findExecutable(QStringLiteral("pdf2ps")).isEmpty());
}
...
bool FilePrinter::cupsAvailable()
{
    ...
    // However, if CUPS is available then QPrinter::numCopies() will always
    // return 1 whereas if CUPS is not available it will return the real number
    // of copies. This behaviour is guaranteed never to change, so we can use it
...
QStringList FilePrinter::printArguments(
    QPrinter& printer, FileDeletePolicy fileDeletePolicy,
    ...
    QPrinter::Orientation documentOrientation, ScaleMode scaleMode)
{
    ...
    if(!pages(printer, pageSelectPolicy, pageRange, useCupsOptions, version)
            .isEmpty())
    {
        argList << pages(printer, pageSelectPolicy, pageRange, useCupsOptions,
                         version);
    }

    if(useCupsOptions &&
       !cupsOptions(printer, documentOrientation, scaleMode).isEmpty())
    {
...
QStringList FilePrinter::copies(QPrinter& printer, const QString& version)
{
    ...
    if(version == QLatin1String("lp"))
    {
        return QStringList(QStringLiteral("-n"))
               << QStringLiteral("%1").arg(cp);
    }
                               ...
                               const QString& version)
{
    if(pageSelectPolicy == FilePrinter::SystemSelectsPages)
    {
        if(printer.printRange() == QPrinter::Selection && !pageRange.isEmpty())
        {
            ...
            if(version.startsWith(QLatin1String("lpr")) && useCupsOptions)
            {
                return QStringList(QStringLiteral("-o"))
                       << QStringLiteral("page-ranges=%1").arg(pageRange);
            }
        }
        ...
        if(printer.printRange() == QPrinter::PageRange)
        {
            if(version == QLatin1String("lp"))
            {
                return QStringList(QStringLiteral("-P"))
                       << QStringLiteral("%1-%2")
                              .arg(printer.fromPage())
                              .arg(printer.toPage());
            }
            ...
            {
                return QStringList(QStringLiteral("-o"))
                       << QStringLiteral("page-ranges=%1-%2")
                              .arg(printer.fromPage())
                              .arg(printer.toPage());
            }
        }
       ...
       !mediaPaperSource(printer).isEmpty())
    {
        return QStringList(QStringLiteral("-o"))
               << QStringLiteral("media=%1,%2")
                      .arg(mediaPageSize(printer), mediaPaperSource(printer));
    }
...
QString FilePrinter::mediaPageSize(QPrinter& printer)
{
    ...
    case QPrinter::Custom:
        return QStringLiteral("Custom.%1x%2mm")
            .arg(printer.widthMM())
...
QStringList FilePrinter::optionOrientation(
    QPrinter& printer, QPrinter::Orientation documentOrientation)
{
    // portrait and landscape options rotate the document according to the
    // document orientation If we want to print a landscape document as one
    // would expect it, we have to pass the portrait option so that the document
    // is not rotated additionally
...
QStringList FilePrinter::optionDoubleSidedPrinting(QPrinter& printer)
{
    ...
    case QPrinter::DuplexNone:
        return QStringList(QStringLiteral("-o"))
               << QStringLiteral("sides=one-sided");
    case QPrinter::DuplexAuto:
        if(printer.orientation() == QPrinter::Landscape)
        {
            return QStringList(QStringLiteral("-o"))
                   << QStringLiteral("sides=two-sided-short-edge");
        }
        else
        {
            return QStringList(QStringLiteral("-o"))
                   << QStringLiteral("sides=two-sided-long-edge");
        }
                                           ...
                                           ScaleMode scaleMode)
{
    ...
    else
    {
        ...
        marginOptions << (QStringLiteral("-o"))
...
QStringList FilePrinter::optionCupsProperties(QPrinter& printer)
{
    ...
    for(int i = 0; i < dialogOptions.count(); i = i + 2)
    {
        ...
        {
            cupsOptions << QStringLiteral("-o")
                        << dialogOptions[i] + QLatin1Char('=') +
                               dialogOptions[i + 1];
        }
    }
