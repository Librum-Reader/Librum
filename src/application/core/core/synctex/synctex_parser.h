/*
    ...
    The author received useful remarks from the __pdfTeX__ developers,
   especially Hahn The Thanh, and significant help from __XeTeX__ developer
   Jonathan Kew.

    ## Nota Bene:

    If you include or use a significant part of the __SyncTeX__ package into a
   software, I would appreciate to be listed as contributor and see
   "__SyncTeX__" highlighted.
*/
...
#define SYNCTEX_VERSION_STRING "1.19"
     ...
     * returns the number of nodes satisfying the contrain. Use code like
     *
     ...
     * the number of nodes satisfying the contrain. Use code like
     *
     ...
     *
     *  page is 1 based
     *  h and v are coordinates in 72 dpi unit, relative to the top left corner
     * of the page. If you make a new query, the result of the previous one is
     * discarded. If you need to make more than one query in parallel, use the
     * iterator API exposed in the synctex_parser_private.h header. If one of
     * this function returns a negative integer, it means that an error
     * occurred.
     *
     ...
     */
    synctex_status_t synctex_display_query(synctex_scanner_p scanner,
                                           const char* name, int line,
                                           int column, int page_hint);
    synctex_status_t synctex_edit_query(synctex_scanner_p scanner, int page,
                                        float h, float v);
     ...
     * name. Conversely, given a file name, synctex_scanner_get_tag will return,
     * the corresponding tag. The file name must be the very same as understood
     * by TeX. For example, if you \input myDir/foo.tex, the file name is
     * myDir/foo.tex. No automatic path expansion is performed. Finally,
     * synctex_scanner_input is the first input node of the scanner. To browse
     * all the input node, use a loop like
     *      ...
    ...
    synctex_node_p synctex_scanner_input_with_tag(synctex_scanner_p scanner,
                                                  int tag);
     ...
     *  These are expressed in TeX small points coordinates, with origin at the
     * top left corner. A box is enclosing itself.
     */
