...
extern "C"
{
        ...
        #define SYNCTEX_PRINTF_FORMAT(si, ftc) \
            __attribute__((format(printf, si, ftc)))
    #else
        #define SYNCTEX_PRINTF_FORMAT(si, ftc)
    #endif

    /*  This custom malloc functions initializes to 0 the newly allocated
     * memory. There is no bzero function on windows. */
    ...
    /*  This is used to log some informational message to the standard error
     *stream. On Windows, the stderr stream is not exposed and another method is
     *used. The return value is the number of characters printed.	*/
     ...
     * name. It might not be the real way of doing things. The return value is
     * an undefined non 0 value when the two file names are equivalent. It is 0
     * otherwise. */
     ...
     *buffer is created, the src is copied and quotes are added. In all other
     *cases, no destination buffer is created and the src is not copied. 0 on
     *success, which means no error, something non 0 means error, mainly due to
     *memory allocation failure, or bad parameter. This is used to fix a bug in
     *the first version of pdftex with synctex (1.40.9) for which names with
     *spaces were not managed in a standard way. On success, the caller owns the
     *buffer pointed to by dest_ref (is any) and is responsible of freeing the
     *memory when done. The size argument is the size of the src buffer. On
     *return the dest_ref points to a buffer sized size+2.*/
    ...
    /*  returns the correct mode required by fopen and gzopen from the given
     * io_mode */
