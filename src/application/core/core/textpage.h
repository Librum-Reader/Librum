...
/*! @class TextEntity
 * @short Represents a piece of text on a TextPage, containing its textual
 * representation and its bounding box.
 *
 * To enable searching and text selection, a generator can give information
 * about the textual content of a Page using a TextPage. A TextPage is created
 * using TextEntity objects. A TextEntity can represent a single
 * character/glyph, a word, a line, or even the whole page.
 *
 * Ideally, every single glyph is represented by its own TextEntity.
 * If the textual representation of a graphical glyph contains more than one
 * character, the TextEntity must contain the whole string which represents the
 * glyph.
 *
 * When the Generator has created the TextPage, and it is added to a Page,
 * the text entities are reordered to words, lines, and paragraphs, to optimize
 * search and text selection. This way, the Generator does not need to care
 * about the logical order of lines or paragraphs.
 *
 * @par Text Selection/Highlighting
 * A TextEntity is the smallest piece of text, which the user can select, or
 * which can be highlighted. That is, if the TextEntity represents a word, only
 * the whole word can be selected. It would not be possible to select a single
 * glyph of the word, because its bounding box is not known.
 *
...
/**
 * @short Represents the textual information of a Page. Makes search and text
 * selection possible.
 *
 * A Generator with text support should add a TextPage to every Page.
 * For every piece of text, a TextEntity is added, holding the string
 * representation and the bounding box.
 *
 * Ideally, every TextEntity describes only one glyph.
 * A "glyph" is one character in the graphical representation, but the textual
 * representation may consist of multiple characters (like diacritic modifiers).
 *
 * When the TextPage is added to the Page, the TextEntitys are restructured to
...
public:
    ...
    {
        AnyPixelTextAreaInclusionBehaviour,  ///< A character is included into
                                             ///< text() result if any pixel of
                                             ///< his bounding box is in the
                                             ///< given area
        CentralPixelTextAreaInclusionBehaviour  ///< A character is included
                                                ///< into text() result if the
                                                ///< central pixel of his
                                                ///< bounding box is in the
                                                ///< given area
    };
     ...
     * @param area If null the search starts at the beginning of the page,
     * otherwise right/below the coordinates of the given rect.
     */
    RegularAreaRect* findText(int searchID, const QString& query,
