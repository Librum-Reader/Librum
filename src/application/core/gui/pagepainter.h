...
public:
     ...
     * @param scaledWidth The requested width of uncropped page in @p
     * destPainter coordinates.
     * @param scaledHeight The requested height of uncropped page in @p
     * destPainter coordinates.
     * @param pageLimits Where to paint in @p destPainter coordinates. (I. e.
     * painter crop.) Should begin at (0, 0).
     */
    static void paintPageOnPainter(QPainter* destPainter,
                                   const Okular::Page* page,
                                   Okular::DocumentObserver* observer,
                                   int flags, int scaledWidth, int scaledHeight,
     ...
     * @param viewPortPoint Which point of the page to highlight, e. g. a source
     * location. @c nullptr to disable.
     */
    static void paintCroppedPageOnPainter(
...
private:
    ...
    /**
     * Collapse color space (from white to black) to a line from @p foreground
     * to @p background.
     */
    static void recolor(QImage* image, const QColor& foreground,
    ...
    /**
     * Inverts luma of @p image using the luma coefficients @p Y_R, @p Y_G, @p
     * Y_B (should sum up to 1), and assuming linear 8bit RGB color space.
     */
     ...
     */
    static void invertLumaPixel(uchar& R, uchar& G, uchar& B, float Y_R,
                                float Y_G, float Y_B);
    /**
     ...
     *
     * @note @p normPath needs to be normalized in respect to @p image, not to
     * the actual page.
     */
    static void drawShapeOnImage(QImage& image, const NormalizedPath& normPath,
     ...
     *
     * @param rect Two NormalizedPoints describing the bounding rect. Need to be
     * normalized in respect to @p image, not to the actual page.
     */
    static void drawEllipseOnImage(QImage& image, const NormalizedPath& rect,
...
public:
     ...
     * @param toNormalizedImage How to transform normalized coordinates of @p a
     * to normalized coordinates of your paint device. (If your paint device
     * represents the whole page, use the unit matrix QTransform().)
     */
    LineAnnotPainter(const Okular::LineAnnotation* a, QSizeF pageSizeA,
                     double pageScale, const QTransform& toNormalizedImage);
...
private:
    ...
    void drawShortenedLine(double mainSegmentLength, double size, QImage& image,
                           const QTransform& toNormalizedPage) const;
    void drawLineEnds(double mainSegmentLength, double size, QImage& image,
                      const QTransform& transform) const;
    void drawLineEndArrow(double xEndPos, double size, double flipX, bool close,
                          const QTransform& toNormalizedPage,
                          QImage& image) const;
    void drawLineEndButt(double xEndPos, double size,
                         const QTransform& toNormalizedPage,
                         QImage& image) const;
    void drawLineEndCircle(double xEndPos, double size,
                           const QTransform& toNormalizedPage,
                           QImage& image) const;
    void drawLineEndSquare(double xEndPos, double size,
                           const QTransform& toNormalizedPage,
                           QImage& image) const;
    void drawLineEndDiamond(double xEndPos, double size,
                            const QTransform& toNormalizedPage,
                            QImage& image) const;
    void drawLineEndSlash(double xEndPos, double size,
                          const QTransform& toNormalizedPage,
                          QImage& image) const;
    void drawLeaderLine(double xEndPos, QImage& image,
                        const QTransform& toNormalizedPage) const;

    template<typename T>
    QList<Okular::NormalizedPoint> transformPath(
        const T& path, const QTransform& transform) const
    {
    ...
    static double shortenForArrow(double size,
                                  Okular::LineAnnotation::TermStyle endStyle);
