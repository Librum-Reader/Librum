    ...
    const Okular::Document* doc)
{
    ...
    while(curPage <= endPage)
    {
        const QList<Okular::FormField*> formFields =
            doc->page(curPage++)->formFields();
        for(Okular::FormField* f : formFields)
        {
            if(f->type() == Okular::FormField::FormSignature)
            {
                signatureFormFields.append(
                    static_cast<Okular::FormFieldSignature*>(f));
            }
        }
    }
    std::sort(signatureFormFields.begin(), signatureFormFields.end(),
              [](const Okular::FormFieldSignature* a,
                 const Okular::FormFieldSignature* b)
              {
...
QString getReadableKeyUsageCommaSeparated(
    Okular::CertificateInfo::KeyUsageExtensions kuExtensions)
{
    return getReadableKeyUsage(kuExtensions,
                               i18nc("Joins the various ways a signature key "
                                     "can be used in a longer string",
                                     ", "));
}
    ...
    const Okular::SignatureInfo& signatureInfo)
{
    ...
    if(signatureStatus == Okular::SignatureInfo::SignatureValid)
    {
        ...
        {
            return i18n(
                "The revision of the document that was covered by this "
                "signature has not been modified;\n"
                "however there have been subsequent changes to the document.");
        }
    }
    else if(signatureStatus == Okular::SignatureInfo::SignatureDigestMismatch)
    {
        return i18n("The document has been modified in a way not permitted by "
                    "a previous signer.");
    }
    else
    {
        return i18n(
            "The document integrity verification could not be completed.");
    }
}

std::pair<KMessageWidget::MessageType, QString>
    documentSignatureMessageWidgetText(const Okular::Document* doc)
{
    ...
    if(isDigitallySigned)
    {
        const QVector<const Okular::FormFieldSignature*> signatureFormFields =
            SignatureGuiUtils::getSignatureFormFields(doc);
        ...
        for(const Okular::FormFieldSignature* signature : signatureFormFields)
        {
            ...
            else
            {
                ...
                if(info.signatureStatus() !=
                   Okular::SignatureInfo::SignatureValid)
                {
        ...
        {
            return { KMessageWidget::Information,
                     i18n("This document has unsigned signature fields.") };
        }
        else if(allSignaturesValid)
        {
            ...
            {
                return { KMessageWidget::Information,
                         i18n("This document is digitally signed.") };
            }
            else
            {
                return { KMessageWidget::Warning,
                         i18n("This document is digitally signed. There have "
                              "been changes since last signed.") };
            }
        }
        else
        {
            return { KMessageWidget::Warning,
                     i18n("This document is digitally signed. Some of the "
                          "signatures could not be validated properly.") };
        }
    }
